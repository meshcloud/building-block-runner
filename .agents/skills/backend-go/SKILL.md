...
One Go module rooted at the repo root (`go.mod` at root; no `go.work`, no per-persona
submodules). Entry points are `cmd/<persona>/main.go` (wiring only); all domain/application
