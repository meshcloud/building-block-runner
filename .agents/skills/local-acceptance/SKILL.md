...
2. **Retire the mux, start bbrunner as ALL.** The mux runs as a compose service; shut it down and
   ...
   sed 's#host\..*internal:8080#localhost:8080#' cmd/bbrunner/runner-config.yml > /tmp/bbrunner-all-config.yml
...
3. **Build images and load them into minikube (no push):** build the `:main` images on the host via
   the single multi-target bake (`task images`, which shares the Go/nix build cache across targets),
   then `minikube image load` them into the cluster's container runtime — nothing is pushed/pulled
   from ghcr. `imagePullPolicy` is already `IfNotPresent` and images are `:main`-tagged, so the loaded
   local image is used. (Building on the host and loading reuses the bake cache; `minikube image
   build` inside the cluster has no cache and re-runs tf's nix install from scratch.)
   ```bash
   # build only the targets you will exercise (run-controller + the per-type images the suite hits):
   TAGS=main nix develop --command docker-buildx bake -f docker-bake.hcl run-controller manual tf
   # add gitlab/azdevops/github targets only if you will exercise those types
   for img in run-controller:main manual-block-runner:main tf-block-runner:main; do
     minikube image load ghcr.io/meshcloud/$img
   done
   # If a run-controller Deployment already exists in the cluster (prior session), force it onto the
   # freshly-loaded image: kubectl rollout restart deployment/run-controller
   ```
4. **Deploy the controller:**
   ```bash
   kubectl apply -f deploy/run-controller/rbac.yaml
   kubectl apply -f deploy/run-controller/deployment.yaml
