...
### 1.1 STOP-gate verification run (2026-07-10, on `refactor/single-go-binary/phase-3-shared-core` branched from `phase-2b-bugfix` @ `0c33fb0`)
...
| # | Result | Notes |
|---|---|---|
| A1 | **verified** | `task test`/`task lint`/`task coverage` all green on `phase-2b-bugfix` (note: the branch is actually named `phase-2b-bugfix`, not `…-bugfixes` as the verification command in the table spells it — a harmless typo in this plan, not a finding). |
| A2 | **FAILED** | `tf-block-runner/internal/{tf,gitsource,tofu}` do not exist; the package is still `tf-block-runner/tfrun` (+ `tf-block-runner/util`, undissolved) with `worker.go`/`singlerunworker.go`/`tfcmd.go` all present. The plan-02 commit (`d2026b4`) is titled *"DDD refactor of the tf runner (phase 2, partial)"* — i.e. phase 2 was never finished on this branch; only some of plan 02's steps landed (globals for run-context and the progress/B10 tracker are done — see A3/A10 — but the package split and the Worker/SingleRunWorker/Engine unification are not). See resolution below. |
| A3 | **PARTIALLY FAILED** | `runInfoContextKey`/`context.Value` smuggling is gone (confirmed removed) and the phase-2 `Decryptor` port + `progress` tracker exist, but `tfrun.AppConfig` (`tf-block-runner/tfrun/config.go:14`) is still a live package-level global (read/written throughout `tfrun` and by tests, e.g. `runapi_status_test.go:88`), and `tfrun.ToInternalWithoutDecryption` (the decryption fork D4 asks plan 02 to kill) still exists (`tfrun/dtos.go:61`, called from `tf-block-runner/main.go:150`). `go-meshapi-client`'s only surviving mutable globals are indeed `runnerName`/`runnerVersion` (matches the assumption). |
| A4 | **FAILED (consistent with A2)** | Gate is ON and green, but the threshold line is still `…/tf-block-runner/tfrun 90` (not `…/internal 90` — the package hasn't moved) at **90.4%** measured — a 0.4pp buffer, not the ≥1pp the assumption predicted. |
| A5 | verified | `-race` is wired in `Taskfile.yml:37-40` and `.github/workflows/ci.yml:163-165` for the tf leg. |
| A6 | verified | `.golangci.yml:160-166`: the `run-controller/controller/` and `go-meshapi-client/` exclusion blocks are present, comment-owned to phase 3. |
| A7 | verified | `git diff main..phase-2b-bugfix -- run-controller/ ':!*_test.go'` is 7 files / 41+/40- — all comment-punctuation (trailing periods), a Makefile→Taskfile doc reference, an embedded-field access simplification (`volume.VolumeSource.ConfigMap` → `volume.ConfigMap`, semantically identical), and an import-line move. No behavior drift. |
| A8 | verified | `.github/workflows/ci.yml:152-158`: `go-meshapi-client` has its own CI matrix leg. |
| A9 | verified (reinterpreted) | The literal grep (`client_test.go` only) under-counts: the 128MiB-cap pin lives in the sibling file `go-meshapi-client/meshapi/artifact_cap_test.go` (`TestDownloadArtifact_OversizedArtifact_RejectedWithMaxSizeError`, `…_ArtifactExactlyAtCap_Succeeds`), same package, with an explicit cross-reference comment in `tf-block-runner/tfrun/cp4_inputs_crypto_test.go:14-15`. Read at the package level (not the single named file) the assumption holds. |
| A10 | verified | No `// FIXME(bug):` markers anywhere in the three modules; `Behavior.str()` returns `"UNKNOWN"` instead of panicking (`behavior.go:18-24`, B12); `RunStatus.Steps` is value-typed `[]StepStatus` with a lock-guarded `clone()` (`runstatus.go:8,65-73`, B6/B10). |

**Resolution for the A2/A3/A4 failure (autonomous, phase does not halt):** the root cause
is one fact, not three independent breaks — phase 2's DDD refactor landed only partially
(its own commit message says so). Completing it is explicitly **phase 2's** scope, not
phase 3's (§5 high-level: "Phase N+1 must not start before phase N's exit criteria hold"),
so phase 3 does not silently absorb that work under a different name. The autonomous
resolution adopted here: **phase 3's step 0 (preflight, §6) is expanded** to first finish
plan 02's outstanding steps — the package split into `internal/tf`+`internal/gitsource`+
`internal/tofu`, the `Worker`/`SingleRunWorker` → one `Engine` collapse, `AppConfig`
de-globalization via injection, and deleting the `ToInternalWithoutDecryption` fork — as a
mandatory precondition, using plan 02 §5/§6's own migration sequence (unchanged), *before*
running plan 03's step 1 onward. Once that precondition holds, every one of plan 03's
steps applies exactly as written (they were authored against that shape). This is recorded
here rather than actually executed in this STOP-gate check, which is scoped to
verification only.
