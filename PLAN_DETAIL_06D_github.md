...
| 8 | **Image.** Per-app `containers/github-block-runner/Dockerfile` + `containers/github-block-runner/runner-config.yml` (§8). | containers/ | `docker build -f containers/github-block-runner/Dockerfile` (no `--target`) + healthz/claim-loop smoke |
