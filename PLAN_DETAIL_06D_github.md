...
| 5 | **Handler + poller.** `NewHandler`/`Execute` + `pollWorkflow` (§4.1/§4.5): the scenario suite (§10.1) — run JSON in → `meshapitest` (plan 03 §5.7) + fake GitHub transcripts out, async/sync/error paths, fake clock driving find window and 30-min timeout. | `internal/github` | scenario suite matches the Kotlin pins |
...
| 8 | **Image.** Per-app `containers/github-block-runner/Dockerfile` + `containers/github-block-runner/runner-config.yml` (§8). | containers/ | `docker build -f containers/github-block-runner/Dockerfile` (no `--target`) + mock-backed container smoke (umbrella §5.6): `docker run` against a `meshapitest` server (plan 03 §5.7, `RUNNER_API_URL`→mock), seed a run, assert the claim→report cycle — not just healthz |
