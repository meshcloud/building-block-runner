...
### 1.1 STOP-gate verification run (2026-07-10, on `refactor/single-go-binary/phase-6a-manual` branched from `phase-5-dispatcher` @ `bb016f8`)

Full-autonomous run (orchestration override): a materially-false assumption is resolved
and recorded, not halted on. `task test` (`-race`), `task lint` (0 issues) and
`task coverage` are all green on the branch point before this check. Results:

| # | Result | Notes |
|---|---|---|
| A1 | **verified (in substance)** | `dispatch.RunHandler.Execute(ctx context.Context, run ClaimedRun) error` (`internal/dispatch/runhandler.go:33-35`) and its doc comment match the contract verbatim (decrypt-per-run, runToken-only reporting, handler-owned timeout, non-nil error = infra failure only). One harmless shape drift: `ClaimedRun.Details` is `*meshapi.RunDetailsDTO` (`dispatch.go:26`), a pointer, not the assumption's bare value type — P4's own pointer-for-nullable-composite rule justifies this, not a gap. |
| A2 | **PARTIALLY FAILED** | `LoopConfig{PollInterval, ClaimBackoff, MaxConcurrent}` (`loop.go:22-32`), injectable `ClaimClassifier` (`dispatch.go:88`, `loop.go:44`) and `Done()`/cadence all present and green (`Test_Loop_NeverExceedsMaxConcurrentRuns`, `Test_Loop_WakeAndTickerRace`, etc. all PASS). But the **persona-facing** knob does not exist yet: no `maxConcurrentRuns` config key or `RUNNER_MAX_CONCURRENT_RUNS` env var anywhere (`grep` across `internal/config`, `cmd/` — zero hits). Only the k8s-controller-only `maxConcurrentJobs` YAML key / `MaxConcurrentJobs` struct field (`cmd/bbrunner/controller_config.go:27`) exists. Standalone in-process personas (06A onward) have no config surface for this yet — expand 06A step 5 to originate it, not assume it exists. |
| A3 | **FAILED** | No `registration:` YAML section, no capability-driven opt-in startup PUT (no WIF) exists anywhere outside the k8s controller's own **mandatory** registration fields (`OwnedByWorkspace`/`DisplayName`/`PublicKey` top-level, required at validation, `cmd/bbrunner/controller_config.go:30-31,56,171-177`) — this is plan-05's own step-8 promise (`PLAN_DETAIL_05_dispatcher.md:704`: "tf `registration:` section + opt-in startup PUT"), never landed: `cmd/tf/main.go` has zero registration/capability wiring (`grep -n "registration\|Capability" cmd/tf/main.go` — only an unrelated comment hit), and no registration-transcript tests exist for a standalone persona. Same recurring "promised, not delivered" pattern phases 3/4/5 already flagged for other items. **06A is the first sub-plan that needs this** (its own step 0/5 already anticipate originating it) — not blocking, but the umbrella's "reusable machinery" framing for this section is currently aspirational, not existing. |
| A4 | **FAILED (known carried debt, phase-5's own A5 finding, unremediated)** | `meshapi.DecryptRunDetails` does not exist. The five-impl-type decrypt branches (`SshPrivateKey`, `AppPem`, `PipelineTriggerToken`, `PersonalAccessToken`, input decryption) live as the **unexported** `decryptRunDetails` in `internal/k8sjob/decryption.go:17-118` — k8sjob-private, not shared into `meshapi` or any handler-consumable package. Not blocking for 06A (manual decrypts nothing, `NoOpBlockRunnerService.kt:22-32` confirms the crypto placeholder), but 06B (gitlab: `pipelineTriggerToken`)/06C (azdevops: `personalAccessToken`)/06D (github: `appPem`) will each need this shared and **must not assume it already is** — flagged forward rather than re-deferred again. |
| A5 | **verified** | `report.Progress`/`RunStatus`/value-typed `Steps []StepStatus` (`runstatus.go:14-29`), `Reporter{Register, Report}` (`reporter.go:13-14`), `ToStatusUpdate` (`dto.go:15`), `Observer` ticker present and explicitly documented as tf-only (`report/observer.go`). |
| A6 | **verified (in substance)** | `config.Api` (`api.go:14`) + `NewAuthProvider` (API-key-wins), `ManagementPort(log, def, aliases…)` (`managementport.go:36`) all present and green (`TestManagementPort_*`, `TestPath_*`, `TestApi_*` all PASS). Naming drift only: `Path`/`Env` are methods on `*Loader` (`env.go:20,46`), not free functions, and there is no `LoadFile` — the loader's entry point is `Loader.Load` (`config.go:85`) — harmless, matching the house pattern of prior STOP-gate checks recording line/name drift as non-blocking. |
| A7 | **PARTIALLY FAILED** | `cmd/tf/main.go` + `cmd/bbrunner/main.go` wiring, `mgmt.NewServer` (`server.go:42`) and `mgmt.RunMetrics` (`metrics.go:20`, `runner_runs_claimed_total`/`_succeeded_total`/`_failed_total`/`runner_poll_errors_total`, `runnerUuid`-labeled) all match. But the plan-05-promised additions `runner_runs_unhandled_total` and `runner_at_capacity_skips_total` do **not** exist under those names: `internal/dispatch/metrics.go` only has the k8s-specific `run_controller_jobs_at_capacity_skips_total` (`:113`), and the D5 fail-fast unhandled-type path (`unhandledtype.go`) reports FAILED but increments no counter at all. 06A step 7 (coverage/tooling gate) needs to originate both metrics fresh, not assume reuse. |
| A8 | **verified** | `tools/coverage/thresholds.txt` has exactly the 7 lines promised (`tf`, `config`, `meshapi`, `report`, `mgmt`, `dispatch`, `k8sjob`, all 90); `exclusions.txt` lists `tf/git.go`, `tf/tfbinaries.go`, `k8sjob/cluster.go` with justifications; `task coverage` measures all 7 ≥ 90% (`dispatch` 97.7%, `k8sjob` 95.7%, others per phase-5's own numbers) and `-race` is on globally. |
| A9 | **verified in substance; literal step unexecutable** | Go's `crypto.MeshCertBasedCrypto.DecryptMeshCertBased` (`meshcertbasedcrypto.go:113-148`) + `MeshSymmetricCrypto.DecryptSymmetric` (`meshsymmetriccrypto.go:61-76`) implement RSA-OAEP-SHA1 key unwrap + AES-128-GCM with a 4-byte big-endian IV-length prefix, matching `MeshCertDecryptionService.kt:34-120`'s `ByteBuffer.int` length-prefix + `AES/GCM/NoPadding` byte-for-byte at the algorithm level (confirmed by direct code comparison, not test-fixture cross-decryption). The prescribed verification step — "cross-decrypt one Kotlin-test fixture ciphertext … in a scratch test" — could not be executed as worded: `MeshCertDecryptionServiceTest.kt` mocks `sut.decrypt(...)` (`every { sut.decrypt(encryptedValue) } returns decryptedValue`, line ~118) rather than embedding a real ciphertext fixture, so there is no fixture ciphertext to cross-decrypt. Go's own `crypto` tests (`Test_CertBasedCrypto`, `Test_SymmetricCrypto`, etc.) all pass. Recorded as a judgment call below (security-relevant claim verified by code reading, not by an executed round-trip). |
| A10 | **verified (in substance)** | `git diff main..phase-5-dispatcher -- '*.gradle' containers/jvm.Dockerfile` is empty (0 lines). `.github/workflows/` is **not** empty-diffed as the literal check demands, but every touched line is the go-side Dockerfile-path/CI-leg reshaping already sanctioned by phases 4/5's own D2/D8/D14 work (`containers/run-controller/Dockerfile` path move, `go-runners-ci`→`go-ci` single-leg collapse) — `jvm-runners-ci`/`jvm-runners-image` and all four JVM matrix legs in `build-images.yml` are untouched. |
| A11 | **PARTIALLY FAILED (already tracked, no new action)** | meshfed-release's `local-dev-stack` SKILL.md still starts manual via `./gradlew :manual-block-runner:bootRun` (unaffected until 06A ships) and the mux ports (`:8300`-`:8304`) match exactly. But its tf-runner instructions (`cd ../building-block-runner/tf-block-runner && go run .`) are stale post phase-4 (that module dir no longer has its own `go.mod`) — this exact drift is already recorded verbatim in this repo's own `CROSS_REPO_TODO.md:54-92`, so no new hand-off note is needed. |
| A12 | **verified** | `containers/run-controller/runner-config.yml:142,147,152,157` sets `SPRING_PROFILES_ACTIVE: kubernetes` for all four Kotlin impl types; `internal/k8sjob/kubernetes.go:667,671,677` injects `RUN_JSON_FILE_PATH`/`RUNNER_UUID`/`RUNNER_API_URL`; `kubernetes_test.go` provides the `kubernetes/fake`-backed manifest goldens phase-5's own A3 finding required. |

**Resolution (autonomous, phase does not halt):** A3, A4 and A7 are the umbrella's own
"shared machinery" promises (registration section, shared decrypt, generic unhandled/
capacity metrics) that plan 05 committed to but did not land — the same rolling-debt
pattern phases 3/4/5 already recorded for other items, now surfacing here as three fresh
instances rather than a re-statement of the old ones (A4 IS the same root cause as
phase-5's own A5, carried forward unremediated; A3/A7 are new instances of the pattern
against different plan-05 promises). None block 06A from proceeding: manual needs no
decryption (A4 only matters from 06B on) and its own step 0/5/7 already anticipate
originating the registration section, the `maxConcurrentRuns` knob (A2), and the two
metrics (A7) fresh — 06A's plan text already reads correctly against today's tree, it
just cannot assume reuse of machinery that does not exist yet. A9's literal verification
step (fixture cross-decryption) is unexecutable because the cited Kotlin test mocks the
decrypt call; substituted with a direct algorithm-level code comparison, which holds.
A10/A11 fail their literal grep/proof-string check only because of unrelated, already-
sanctioned changes elsewhere in the same file/doc — the substance they guard (Kotlin
build tooling untouched; meshfed-release manual startup path unaffected) holds. No
umbrella or sub-plan text needs revision as a result of this run; the individual gaps
are recorded above so 06A/06B onward do not silently assume the missing machinery
exists.
