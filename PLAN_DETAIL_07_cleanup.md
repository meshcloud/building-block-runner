...
### 1.1 STOP-gate verification run (2026-07-10, on `refactor/single-go-binary/phase-7-cleanup`
branched from `phase-6d-github` @ `6a26562`, fast-forwarded onto the local worktree that
already carries 06B/06C/06D — the fan-out siblings for those sub-phases had already
landed their commits by the time this check ran)

Full-autonomous run (orchestration override): a materially-false assumption is resolved
and recorded, not halted on. `task test` (`-race`), `task lint` (0 issues after clearing
a stale shared `golangci-lint` cache that mislabeled file paths from a sibling worktree —
environmental, not a code finding) and `task coverage` are all green on the branch point
before this check (one `internal/tf` coverage run flaked under full-suite concurrency
load; re-run in isolation and as part of the full suite both passed — not a real gap).

| # | Result | Notes |
|---|---|---|
| A1 | **verified** | `task test -race`/`task lint`/`task coverage` green; module at repo root, no `go.work`; `cmd/bbrunner` registers all five fit personas (`tf`,`manual`,`gitlab`,`azdevops`,`github`) plus itself as the superset = six images. |
| A2 | **FAILED — structural, not residue.** `build.gradle`/`gradlew`/`gradle/`/`block-runner-core/` and all four Kotlin module dirs (`manual-block-runner`, `gitlab-block-runner`, `azure-devops-block-runner`, `github-block-runner`) still exist; `containers/jvm.Dockerfile`/`entrypoint-jvm.sh` still exist; `ci.yml` still has `jvm-runners-ci`/`jvm-runners-image`; `flake.nix` still has `ktlint`; `.claude/settings.json` still has the ktlint hook. Root cause: `PLAN_DETAIL_06D_github.md` §12's own step 11 ("JVM endgame") was never executed on `phase-6d-github` — that branch only ported the github runner (steps 1-9ish) and stopped short of its own module-removal/Gradle-shrink exit criteria (steps 10-11). This is **not** one of this plan's own ledger items (L1-L40, §4) — every ledger row assumes the bulk JVM deletion already happened and only owns *residue* (leftover grep hits, doc mentions, `.editorconfig` sections). Nothing currently in §4/§10 performs the actual `git rm` of the Gradle tree or the Kotlin module dirs. |
| A3 | **verified (with expected, plan-sanctioned collapse)** | `internal/{tf,meshapi,crypto,config,report,dispatch,k8sjob,manual,gitlab,azdevops,github,build,meshapitest}` exist; no separate `gitsource`/`tofu` packages — D11 explicitly permits "one cohesive package" for tf if the sibling-package seams don't prove real, which is what happened (`internal/tf/git.go`, `internal/tf/tfbinaries.go`). `meshapitest` (D6) and `internal/resources` (test fixtures) are expected additions the assumption text just didn't enumerate. `internal/controller` is gone, as expected. |
| A4 | **verified** | `tools/coverage/thresholds.txt`/`exclusions.txt` match exactly (three exclusions: `internal/tf/git.go`, `internal/tf/tfbinaries.go`, `internal/k8sjob/cluster.go` — the plan's `gitsource/git.go`+`tofu/tfbinaries.go` wording is pre-collapse naming, same files); `task coverage` PASS on every gated package (tf 90.2%, config 97.2%, meshapi 92.1%, report 98.8%, mgmt 96.4%, dispatch 97.7%, k8sjob 95.7%, manual 97.6%, gitlab 92.1%, azdevops 93.2%, github 91.3%). |
| A5 | **PARTIALLY FAILED, already anticipated by this plan's own ledger.** Zero live `FIXME(bug)` markers (one grep hit is prose in a doc-comment explaining they're gone, not a marker) — L5 holds. But the "category 2" temporary `.golangci.yml` exclusion block (errcheck/forcetypeassert/makezero on `internal/(meshapi|crypto|config|report|meshapitest)/`) is still present verbatim — L6 does **not** hold yet. This is already a **VERIFY, fix-if-found** row in §4 (L6) and step 1 of §10's own migration sequence ("Verification sweep... any failure is triaged... structural is a STOP" — but L6 residue is explicitly anticipated, not structural); no plan update needed, just execute step 1 as written. |
| A6 | **verified** | Alias/deprecation mechanics present in `internal/config` + persona `main.go`s; alias precedence tests (`TestPath_OnlyDeprecatedAliasSet_UsedAndWarns`, `TestManagementPort_OnlyDeprecatedAliasSet_UsedAndWarns`, etc.) all PASS. |
| A7 | **verified** | Diffed `internal/dispatch/metrics.go`'s 13 `run_controller_*` metric names byte-for-byte against `main:run-controller/controller/metrics.go` — identical, zero renames. (The assumption's informal `runner_runs_unhandled_total`/`runner_at_capacity_skips_total` names don't appear verbatim anywhere — closest is `run_controller_jobs_at_capacity_skips_total` — but D12 only forbids *renaming an existing* metric; new-metric naming choices during phases 3-6 are not a D12 violation, and no prior existing name was touched.) |
| A8 | **verified** | Phase-6 handler packages (manual/gitlab/azdevops/github) + meshapi's new decrypt files are `log/slog`; `config`/`report`/`mgmt`/`dispatch`/`k8sjob`/`tf` (+ `meshapi/auth.go`) are still `*log.Logger` — exactly the umbrella-§10.12 interim split this plan's own §8 is scoped to finish. |
| A9 | **FAILED, direct consequence of A2 (not an independent gap).** `ci.yml`'s test/coverage job is `go-ci` (not `go-runners-ci`), still carries `jvm-runners-ci`/`jvm-runners-image`, and `build-images.yml` still passes `RUNNER_MODULE`/uses `jvm.Dockerfile` for four legs — none of that collapses to the assumed six-`runner.Dockerfile`-leg shape until the A2 JVM teardown happens (which is exactly what this phase's own §5 CI reshape is scoped to do, per D14: "CI reshaped only in the last phase"). One part is independently true regardless: **no CI lint job exists yet** (`grep -rn golangci .github/workflows/` — empty), matching D14/plan-00's "no CI lint job until phase 7." |
| A10 | **verified (path only, substance holds)** | The single-run glue lives in `cmd/tf/main.go` (not `runner/persona_tf.go` — path drift from the D2 root-module move, expected) as `executeSingleRun`, still separate from the `tf` handler's own `Execute` path — matches "parallel glue, not yet unified," exactly what L15/step 5/STOP-E targets. |
| A11 | **verified** | `internal/k8sjob/decryption.go`'s `decryptFailure` implements `dispatch.SilentDispatchFailure` (kubernetes.go:86-93) — decrypt errors still produce no status report, matching the pinned transcript-empty quirk L14/step 4/STOP-D targets. |
| A12 | **verified** | `containers/run-controller/runner-config.yml` still ships `SPRING_PROFILES_ACTIVE: kubernetes` in all four pipeline-runner job templates (lines 142/147/152/157) — L12's flip is still pending, as expected. |
| A13 | **FAILED, already a tracked cross-repo TODO (not a new discovery).** `meshfed-release/.agents/skills/local-dev-stack/SKILL.md` still starts the manual runner via `./gradlew :manual-block-runner:bootRun` (JVM), not `go run . manual-block-runner` — direct consequence of A2 (06A/06D never flipped this repo's doc because the Kotlin module they'd be retiring is still there). Already recorded in this repo's `CROSS_REPO_TODO.md` (the SKILL.md manual-runner-block replacement, "lock-step, §15") and covered by this plan's own L21/L22 ledger rows (§10 step 12, STOP-F) — no plan update needed. |
| A14 | **verified (expected non-blocking state)** | `run-controller/k8s/{deployment.yaml,rbac.yaml}` exist (not deleted) at their pre-phase-4 path, not yet moved to `containers/run-controller/k8s/` — exactly the "A14 failing is not a STOP... becomes a step-6/L18 task either way" case this plan already documents. |

**Resolution (A2 — the one gap this plan's own text did not anticipate):** `PLAN_DETAIL_06D_github.md` §12's "JVM endgame" (step 11: delete `block-runner-core/`, all Gradle/JVM machinery, the four Kotlin module dirs, `containers/jvm.Dockerfile`/`entrypoint-jvm.sh`, the `jvm-runners-*` CI jobs, the ktlint toolchain/hook, plus the layout-forced README/`.editorconfig`/`.gitignore` edits its own inventory names) never ran. Every downstream phase-7 ledger row (§4) and migration step (§10) — the CI reshape (§5, L1/L2), the README rewrite (L7), `.editorconfig` (L16), the gradle/kotlin grep gate (L20), the `.gitignore` residue check (L19) — is written assuming that teardown is already done and only owns what's left over once it is. Rather than rewrite this plan's scope (the ledger already correctly anticipates every *residual* piece), this run **adds it to step 0** of §10's migration sequence as an explicit catch-up sub-step, executed with 06D §12's own inventory as the authoritative checklist, before step 1's verification sweep runs (so L19/L20's grep gates have something real to verify against instead of vacuously passing on an unstarted teardown). This is scoped as a plan update per STOP-A's own prescribed remedy ("update this plan first... then resume") — the teardown itself is substantial, mechanical, already-specified implementation work properly belonging to the phase-7 implementation pass, not this verification pass.
...
|---|---|---|---|
| 0 | **Preflight.** Run all §1 verifications on the phase-6d branch; branch `phase-7-cleanup`. Record: coverage numbers, A13 readiness markers, A14 manifest location. **Then, since A2 failed (§1.1): execute `PLAN_DETAIL_06D_github.md` §12's "JVM endgame" (its own step 11, never run) — delete `block-runner-core/`, the four Kotlin module dirs, all Gradle/JVM machinery (`*.gradle`, `gradlew*`, `gradle/`), `containers/jvm.Dockerfile`/`entrypoint-jvm.sh`, the `jvm-runners-*` CI jobs and `RUNNER_MODULE` build-arg legs, the `ktlint` flake/`.claude/settings.json` hook — using 06D §12's inventory as the checklist, before any later step relies on it being gone.** | Gradle/Kotlin tree, `containers/jvm.Dockerfile`+`entrypoint-jvm.sh`, `.github/workflows/*`, `flake.nix`, `.claude/settings.json` | A1–A14 verified (STOP-A); `ls *.gradle gradlew 2>/dev/null` empty; `go build ./...`/`task test` unaffected (no Go code depended on the JVM tree) |
