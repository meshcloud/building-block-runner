...
func buildGithubSupersetHandler(conn supersetConn) (dispatch.RunHandler, error) {
	...
	return github.NewHandler(github.Config{}, github.HandlerDeps{
		Reporters: github.NewReporterFactory(conn.ApiURL, conn.RunnerUuid, id, conn.Log),
		Log:       conn.Log,
	}), nil
}

// runGithubPolling forces the github-block-runner type in-process (`bbrunner github`) for
// local-dev / the mux replacement. It runs the SAME polling wiring as the fit
