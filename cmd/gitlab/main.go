// Command gitlab is the gitlab-block-runner type binary: a fit binary linking
...
// in the cmd/bbrunner superset (bbrunner gitlab). This is wiring only (package main).
package main
...
func run() int {
	...
	return runmode.Main(singleRun, runmode.Runner{
		...
		SingleRun: func(ctx context.Context) int {
			return gitlab.RunSingleRun(ctx, log, cfg, id)
		},
