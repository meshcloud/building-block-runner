// Command manual is the manual-block-runner type binary: a fit binary linking
...
// in the cmd/bbrunner superset (bbrunner manual). This is wiring only (package main).
package main
...
func run() int {
	...
	return runmode.Main(singleRun, runmode.Runner{
		...
		SingleRun: func(ctx context.Context) int {
			return manual.RunSingleRun(ctx, log, cfg, id)
		},
