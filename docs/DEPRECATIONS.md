...
## 3. `blockrunner:` YAML block → flat, top-level keys
...
## 4. `blockrunner.privateKey` / `blockrunner.privateKeyFile` → flat `privateKey` / `privateKeyFile`
...
## 5. `blockrunner.debugMode` → flat `debugMode`

The manual runner's debug-mode toggle. Applies to `manual` only (the one type with a debug-mode concept). Mirror of
§4: **warn-and-use** for manual, **warn-and-ignore** for gitlab/azdevops/github. `config.BlockRunnerCompat`'s own doc
comment promises exactly this ("manual-only; other types warn-and-ignore").

## 6. `VERSION` env var → the compiled-in build version
...
## 7. `api.user` — REMOVED (use `api.username`)

Historically tf accepted `user:` as an alias for the Basic-auth username while the controller used `username:`; both
coexisted on `config.Api`. The `user:` spelling (and the `config.Api.User` field) is now **removed** — every type uses
the canonical `api.username` (see [§ tf `api.user` YAML key removed](#️-tf-apiuser-yaml-key-removed) under Behavior
changes). A mounted config still carrying `user:` no longer sets the username (yaml.Unmarshal silently drops it), so
tf falls back to the compiled-in dev default and, in production, an operator must switch to `username:` (or the
unaffected `RUNNER_API_USERNAME` env).

## 8. `/app/tfrunner` image path — no log
...
The four HTTP-only images (`manual`/`github`/`gitlab`/`azure-devops`), together with the run-controller, now build
from ONE multi-target root `Dockerfile` (via `docker-bake.hcl` / `task images`) and place their binary at the
**uniform** in-image path `/app/runner` (it was previously `/app/<name>-block-runner`). Unlike tf's `/app/tfrunner`,
that per-image path was never a documented `command:` override target — customers reference these by *image name* —
so this is a no-op for every normal deployment. Noted here for anyone who hard-coded the old in-image binary path in
a custom `command:`.

## 9. Prometheus metric names — no rename has happened
...
## 10. `PUSH_GATEWAY_URL` — new config surface, opt-in, off by default
...
## 11. tf's `runnerUuid:` — REMOVED (use `uuid:`)

The tf runner's YAML key for its own runner identity was briefly aliased as `runnerUuid` → `uuid`. That alias (and its
decode-only `RunnerUuidLegacy` field) is now **removed**; tf uses only the canonical `uuid:` key, matching every other
type. See [§ tf `runnerUuid:` YAML key removed](#️-tf-runneruuid-yaml-key-removed) under Behavior changes. `RUNNER_UUID`
(the env override) is unaffected.

## Uniform wording

Every warning in §§1–6 is emitted through one shared helper
...
### ⚠️ tf `api.user` YAML key removed

tf's Basic-auth username is now read only from the canonical `api.username` YAML key; the legacy tf-only `user:` alias
(and the `config.Api.User` struct field) is removed. Applies to `internal/tf` (the sole user of the alias — the other
four types always used `username:`). Now that tf converges on the shared `config.Api`, a single spelling is enough, and
the alias field / `username()` normalization it required across `NewAuthProvider`/`Validate` are gone.

**Impact.** A production tf `runner-config.yml` that still sets `api.user:` (Basic auth) will silently stop supplying
the username — switch it to `api.username:`. The `RUNNER_API_USERNAME` env override is unaffected. This alias was a
pre-release-only surface; removing it in the same consolidation avoids shipping it into a release.

### ⚠️ tf `runnerUuid:` YAML key removed

tf's runner identity is now read only from the canonical `uuid:` YAML key; the `runnerUuid:` alias (introduced only
during this consolidation branch, decode-only via `RunnerUuidLegacy`) is removed in the same pre-release
consolidation, before it ever shipped. Applies to `internal/tf`.

**Impact.** Effectively none in practice (the alias never shipped in a release). A config file carrying only
`runnerUuid:` now falls back to the shared dev-default uuid instead of picking up the legacy value. `RUNNER_UUID` env is
unaffected.

### ⚠️ github default `maxConcurrentRuns` raised 1 → 3

The github runner's default in-process concurrency cap changes from 1 to the shared default 3 (`config.Default
MaxConcurrentRuns`), matching `manual`/`gitlab`/`azdevops`/`tf`. github no longer carries a per-type override; it embeds
the shared `config.BaseConfig` like every other type.

**Impact.** A github poll runner left on the default now dispatches up to 3 workflow runs concurrently instead of one
at a time. github's workflow-run correlation window is heuristic, so operators who need the old serial behavior (to
avoid concurrent-dispatch cross-tracking of the same workflow file) must set `RUNNER_MAX_CONCURRENT_RUNS=1` /
`maxConcurrentRuns: 1` explicitly.

### Shared `uuid` / `api` / `maxConcurrentRuns` now flow through `config.BaseConfig`

The dispatcher-owned shared config (runner uuid, meshfed API/auth section, in-process concurrency cap) is consolidated
into one `config.BaseConfig` (runtime) / `config.BaseFileConfig` (yaml, embedded inline) that every runner type embeds,
resolved by one shared `config.ResolveBase`. tf in particular now uses the same `config.Loader` machinery as the four
HTTP types instead of its bespoke `os.Getenv` loader.

**Impact.** Not operator-breaking beyond the three ⚠️ entries above. The YAML keys (`uuid`, `api.*`,
`maxConcurrentRuns`) and the `RUNNER_*` env overrides are otherwise unchanged. One consequence of tf's convergence:
like the other types, tf now seeds the shared compiled-in dev uuid (`config.DefaultRunnerUuid`) when none is
configured, so a tf config with no `uuid:` boots against the local-dev identity instead of failing the uuid-required
check (real deployments always set `RUNNER_UUID`).

### ⚠️ tf poll runner no longer fails fast without standing auth; shares the compiled-in dev API defaults

A single-type/standalone tf poll runner (`bbrunner -tags type_tf`, or `bbrunner tf`) no longer requires
operator-supplied standing credentials to boot. The former `RequireStandingCredentials` gate (which failed startup
with an "authentication required in polling mode" error when neither Basic nor API-key auth was configured) is
**removed**. tf now applies the same compiled-in dev-local API defaults as the other four fit types — `bb-api` /
`guest` Basic auth (`config.DefaultApiUsername` / `config.DefaultApiPassword`) — so a zero-config tf dispatcher polls
the local meshfed-API out of the box, exactly like `manual`/`gitlab`/`azdevops`/`github`. An explicit file/env value
(`RUNNER_API_USERNAME`/`RUNNER_API_PASSWORD`/`RUNNER_API_CLIENT_ID`/`RUNNER_API_CLIENT_SECRET`) still wins.

The compiled-in dev API **url** default also moved: from the per-type dev-mux ports (`http://localhost:8300`–`8304`)
to the direct local meshfed-API endpoint `http://localhost:8080` (`config.DefaultApiUrl`), shared by the superset and
every single-type dispatcher.

**Impact.** Local-dev-only surface. In production every deployment already supplies its own uuid/api/auth via env, so
the compiled-in defaults are never reached and behavior is unchanged. The only observable change is for a
zero-config local tf poll runner, which now boots against `bb-api`/`guest` at `:8080` instead of failing fast.

### ⚠️ Baked private keys are removed from the tf/github/azure-devops fit images

The `tf-block-runner`, `github-block-runner` and `azure-devops-block-runner` images no longer bake the
well-known local-dev RSA private key into their shipped `runner-config.yml`. These three types decrypt
(appPem, sensitive inputs, tf's `RunnerUuid`-scoped ciphertexts) in standalone polling mode, so they
previously shipped a working dev key out of the box; `manual` and `gitlab`'s per-impl config were
already key-less. `config.ResolvePrivateKey`'s resolution order is otherwise unchanged: env
(`RUNNER_PRIVATE_KEY_FILE`) > `privateKeyFile` > inline `privateKey` > nothing.

**Impact (⚠️ potentially breaking).** Standalone-polling one of these three images with no
operator-supplied key now fails fast at startup with a "private key" error, instead of silently
decrypting off the baked dev key. Keys are now operator-supplied only for every type, matching the
already-key-less `manual`/gitlab-per-impl images. This does not affect the `run-controller`
(`cmd/bbrunner`, `-tags k8s`) image or superset build — the claim-boundary decryptor there still uses
the magic key baked into `cmd/bbrunner/runner-config.yml` (the controller's own config), unchanged.

### ⚠️ `RUNNER_BASE_CONFIG_FILE` and the shared config base layer are removed

The base+per-impl deep-merge config-loading mechanism is gone: `config.Loader.Load` now takes one
path, not a `(basePath, perImplPath)` pair, and the former shared base file `containers/runner-config.yml` (which
gitlab alone wired up, carrying the well-known local-dev private key) is deleted along with gitlab's
`defaultBaseConfigFile` constant and its `RUNNER_BASE_CONFIG_FILE` env alias. This is removed outright,
not aliased — there is no replacement env var, since the mechanism it pointed at no longer exists.
The within-file `blockrunner:` compat block (`BlockRunnerCompat.ApplyShared`, §3–§5 above) is a
separate, still-supported mechanism and is unaffected.

**Impact.** Breaking only for anyone actually setting `RUNNER_BASE_CONFIG_FILE` (an undocumented
surface outside this repo's own local-dev containers). gitlab operators who relied on it for the
well-known local-dev key must now supply their own key via `RUNNER_PRIVATE_KEY_FILE` /
`privateKeyFile` / `privateKey`, the same as every other type.

**Local-dev-only note: unified runner uuid.** The five per-type local-dev default uuids (one baked
into each of `manual`/`gitlab`/`azdevops`/`github`/tf's fit config) are collapsed into one shared
value, `98520496-627d-43e6-82da-ce499179ff3f`, baked into the single shared fit `cmd/runner-config.yml`
(and the controller's `cmd/bbrunner/runner-config.yml`) alike. This is a local-dev-only convenience, not a production default; whether meshStack itself
tolerates concurrent claims from multiple runner processes sharing one runner identity is still an
open verification item (see PLAN.md), so treat this as a caveat on local multi-type dev setups, not a
guarantee.

### ⚠️ Single-run mode is now auto-detected solely from `RUN_JSON_FILE_PATH`; `EXECUTION_MODE`/`SPRING_PROFILES_ACTIVE` no longer trigger it

`EXECUTION_MODE` (tf's own single-run toggle) and `SPRING_PROFILES_ACTIVE` containing `kubernetes` (the four ported
types' toggle) are removed as single-run triggers. Single-run mode is now driven by one unambiguous signal:
`RUN_JSON_FILE_PATH` set and resolving to a readable, non-empty run file. `KUBERNETES_SERVICE_HOST` was never a
candidate signal — every pod (a single-run Job and a long-running in-cluster polling Deployment alike) sets it, so
only the mounted run file distinguishes the two. A `RUN_JSON_FILE_PATH` that is set but missing or empty now fails
the process fast, rather than silently falling back to polling.

**Impact (⚠️ potentially breaking).** An operator whose Job template still sets `EXECUTION_MODE`/
`SPRING_PROFILES_ACTIVE=kubernetes` without mounting the run file at `RUN_JSON_FILE_PATH` no longer gets single-run
behavior — the runner now polls instead. The run-controller's own Job template already mounts the run file, so it is
unaffected. Both env vars are removed outright, not aliased — with the five per-type binaries gone (next entry),
neither side of the old `SPRING_PROFILES_ACTIVE`→`EXECUTION_MODE` mapping exists anymore.

### ⚠️ The five per-type binaries (`cmd/tf`/`manual`/`gitlab`/`azdevops`/`github`) are replaced by build-tag variants of `cmd/bbrunner`

The five standalone `cmd/{tf,manual,gitlab,azdevops,github}` binaries are removed. Each type is now a `-tags type_X`
build of the single `cmd/bbrunner` module, linking only that type's handler (plus a no-tags superset build linking
all types, and the existing `-tags k8s` controller build linking none). This is a build/packaging change only; the
Dockerfile and CI wiring to produce the per-type images from these new build tags is tracked separately (the "One
multi-target Dockerfile" item), not part of this change.

**Impact (⚠️ potentially breaking).** Nothing for an operator consuming the published per-type images by name — the
image identity is unchanged, only what builds it. Anyone building a `cmd/tf`/`cmd/manual`/`cmd/gitlab`/`cmd/azdevops`/
`cmd/github` binary directly (a custom build script, a fork) must switch to `go build -tags type_X ./cmd/bbrunner`.
...
### CUSTOM_CA_CERTS_PATH import now happens from the Go binary; the shell entrypoint is removed

`CUSTOM_CA_CERTS_PATH` import is now done from the Go binary itself: `internal/catrust.RootCAs`
seeds every runner type's process-wide HTTP transport with the system pool plus any PEMs found under
that path, and `internal/catrust.SyncSystemStore` additionally updates the on-disk trust store so
tf's subprocesses (tofu/git/curl/aws-cli) see the same custom CAs. Previously this was done by the
shell entrypoint's (`entrypoint-go.sh`) `update-ca-certificates` call before exec'ing the binary.

**Impact.** Effectively non-breaking. The Dockerfile consolidation (single multi-target root
`Dockerfile` + `docker-bake.hcl`) has now **removed `entrypoint-go.sh`** entirely — the in-binary
`catrust` import is the sole CA-setup mechanism. This is the one behavior change in that slice.
`catrust.RootCAs` already covers all outbound-HTTPS trust for every image, and tf's subprocess trust
is covered by `catrust.SyncSystemStore` (the on-disk store); the four HTTP images have no subprocess
CA consumers, so nothing that previously worked stops working. The scratch runtime for the HTTP fit
images + run-controller has no shell anyway, so a shell wrapper was no longer viable there.

**Config relocation (path-internal only, not an operator surface).** The same slice relocated the
baked config sources into `cmd/`: the shared fit config → `cmd/runner-config.yml`, the controller
config → `cmd/bbrunner/runner-config.yml`, tf's `known_hosts` → `cmd/tf/known_hosts`. The baked
in-image path (`/app/runner-config.yml`) and the `RUNNER_CONFIG_FILE` override are unchanged, so this
is invisible to operators — only the in-repo source location moved.
...
decision (`observability.PushRunMetrics`, see entry 10 below). `ExecuteRun`'s error is, by design, scoped to
...
`runner_runs_succeeded_total`, and (only if `PUSH_GATEWAY_URL` is set — off by default, entry 10 below) leaves its
