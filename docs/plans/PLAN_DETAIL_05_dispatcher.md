...
### 1.1 STOP-gate verification run (2026-07-10, on `refactor/single-go-binary/phase-5-dispatcher` branched from `phase-4-single-binary` @ `6fed28f`)
...
| # | Result | Notes |
|---|---|---|
| A1 | **verified** | Module at repo root (`go.mod:1`), no `go.work*` files, `cmd/{bbrunner,tf}` present, `internal/{build,config,controller,crypto,meshapi,meshapitest,mgmt,report,tf}` present. `task test` (`-race`), `task lint` (0 issues) and `task coverage` all green. Package list differs from the assumption's literal enumeration — no `gitsource`/`tofu` split (`tf` stayed one package, sanctioned by D11's own escape hatch: "one cohesive package is acceptable otherwise") — a naming mismatch, not a gap. |
| A2 | **verified** | `internal/controller/controller.go` has `JobManager` (`:18-20`), `drainRuns`/`availableCapacity`/`processNextRun`/`reportRunFailure` (`:122,148,167,231`) byte-for-byte matching §3.1's citations; `KubernetesClient` present in `kubernetes.go`. |
| A3 | **FAILED** | No k8s Job/Secret/SA manifest goldens exist anywhere in the repo: `grep -rln "kubernetes/fake\|fake.NewSimpleClientset"` over all `*.go` returns nothing. The 38 `func Test` hits in `internal/controller/*_test.go` (matching plan-04's own count) are all loop/capacity/claim/registration/decryption tests against a hand-rolled `fakeJobManager` — **`kubernetes.go` (709 lines: Job/Secret/SA/WIF/volume/size-guard/`CountActiveJobs` construction) has zero test coverage**, hermetic or otherwise. Worse, `KubernetesClient.clientset` is typed as the **concrete** `*kubernetes.Clientset` (`kubernetes.go:27,38`), not the `kubernetes.Interface` `k8s.io/client-go/kubernetes/fake.NewSimpleClientset()` returns — so no `kubernetes/fake` test can even be written against today's shape without first widening that field to the interface. Claim/registration transcript tests (node-id, media types, `StatusUpdateDTO`) **do** exist and are green (`registration_test.go`, `dtos_test.go`) — that half of A3 holds. |
| A4 | **FAILED** | No `Engine` type exists anywhere in `internal/tf` (`grep -rn "type Engine\|func (e Engine)"` — zero hits). The code is still exactly §3.2's "current state": `manager.go` (`DefaultRunManager`, `FAILED_WORKER_DELAY=60s`/`NORUN_WORKER_DELAY=10s` at `:13-14`, single worker + `[WORKER-%03d]` prefix at `:74-90`, `atomic.Bool shutdownCalled` at `:32`), `worker.go` (`handleFetchRunError` incl. the chunked-transport string match at `:92`), `singlerunworker.go` (`SingleRunWorker.ExecuteRun`). This is the identical plan-2/3 DDD-refactor debt plan 04's own §1.1 already found and flagged ("three-phases-in-a-row deferral pattern") — now a fourth instance. Unlike A3, this one is **not** blocking in the same way: `SingleRunWorker.ExecuteRun` already has the shape phase 5 needs from an "Engine" (per-run decrypt via `Decryptor`, per-run `RunApi` with runToken-only auth via `SetRunToken`, per-run working dir) — see Resolution. |
| A5 | **PARTIALLY FAILED** | `meshapi.RunClient`/`RunnerClient`/`HttpError`/`WhitelistedPosts` all exist and match (`client.go`, `runner_client.go`, `retry.go:40-70`). `meshapi.DecryptRunDetails` does **not** exist — decryption of the five impl-type branches is still `controller.DecryptRunDetails` in `internal/controller/decryption.go` (unmoved, unshared with tf's own `internal/tf/decryptor.go` + `ToInternalWithoutDecryption`, `dtos.go:61` — still a live fork, matching plan-03's own A3 finding for itself). `NodeId` is a bare `string` parameter throughout `meshapi.client.go`, not a named type. |
| A6 | **PARTIALLY FAILED** | `report.Progress`/`RunLog`/`Observer`/value-typed `Steps []StepStatus` all present and match; `-race` is ON globally (`Taskfile.yml:20-22`, `ci.yml:168`), not just the tf leg. But "zero package-level mutable state anywhere" is false: `tf.AppConfig` (`internal/tf/config.go:15`), `controller.AppConfig`/`controller.DiscoveredOidcIssuer` (`internal/controller/config.go:13,16`) and `controller.UseTestClient` (`internal/controller/runapi.go:13`) are all still live package-level globals — the plan-02/03 de-globalization (P3) never happened, carried forward unchanged from phase 4's own A4 finding. `report.ExecutionStatus` already has `ABORTED` defined (`executionstatus.go:18`, ahead of schedule, unused so far) — a positive, not a gap. |
| A7 | **verified** | `internal/mgmt/metrics.go:20-59` — `RunMetrics` with all five named series, `runner_uuid`-labeled. |
| A8 | **PARTIALLY FAILED** | `tools/coverage/thresholds.txt` has 5 lines (`tf`, `config`, `meshapi`, `report`, `mgmt`, each 90), not 8 — `crypto` is still ungated (carried-forward debt, consistent with A5/A6's decryption-move deferral); `internal/controller` is indeed ungated as assumed. `exclusions.txt` names `internal/tf/git.go` and `internal/tf/tfbinaries.go` (not `gitsource/`·`tofu/` paths — consistent with A1's no-split finding). Measured: tf 90.2%, config 96.5%, meshapi 90.6%, report 99.2%, mgmt 96.4% — all comfortably ≥90. |
| A9 | **verified** | `SingleRunWorker.ExecuteRun` (`singlerunworker.go:53-114`) returns a non-nil error **only** for `registerErr` (a pre-mutation registration failure, written before `wg.Done()` per the B11 comment at `:94-99`) or the pre-flight `os.MkdirAll`/`os.MkdirTemp`/`initRunContextInfo` failures — once `workRoutine` starts applying, its outcome is reported as a terminal status, not propagated as a Go error. `cmd/tf/main.go:167-171` maps that error to `os.Exit(1)`. Exactly the 2b-R12 conditional form. |
| A10 | **verified** | `internal/tf/tfbinaries.go:32-33` (`mutex sync.Mutex`, `testMode bool`), `:41-42`/`:59-60` (construction), `:80-81` (`testMode` bypass before the mutex) — same pattern, line numbers shifted by ~1 from the citation (harmless). |
| A11 | **verified** | `internal/tf/runapi.go:57-58` (`SetRunToken`/`ClearRunToken` on the seam interface), `:83-89` (impl), `worker.go:65` (`ClearRunToken()` after execution) — the single shared mutable token slot is confirmed still in place. |
| A12 | **verified (in substance)** | `.golangci.yml` keys depguard rules on file-glob paths (`internal/mgmt/**/*.go`, `internal/controller/**/*.go`, …) rather than literal import-path prefixes as the assumption's parenthetical spells it, but the effect is identical; only the `mgmt` and `controller` (+`cmd/bbrunner`) groups may import `github.com/prometheus/client_golang` (`.golangci.yml:90-131`). |

**Resolution (autonomous, phase does not halt):** A3, A4, A5, A6 and A8 are the same root
cause phase 3's and phase 4's own STOP-gate checks already found and named — the plan-2/3
DDD refactor (`Engine` unification, `AppConfig` de-globalization, shared `Decryptor`/
`DecryptRunDetails`) landed partially and the debt keeps rolling forward un-remediated.
This is now the **third** consecutive STOP-gate check to record it (phase 3 §1.1, phase 4
§1.1, this plan) — the underlying incomplete work itself has now rolled unremediated
through four phases (2, 3, 4, 5). Per phase 4's own precedent ("none of phase 4's own steps require that debt paid
down first"), each finding is triaged individually rather than blanket-deferred:

- **A3 (blocking for step 3 / §13's gate claim):** the assumption that "the A3 goldens
  already prove [`k8sjob` is] hermetically testable" (§13) is false — those goldens do not
  exist. Step 3 ("Package split") is expanded to (a) widen `KubernetesClient.clientset`
  from the concrete `*kubernetes.Clientset` to the `kubernetes.Interface` it already
  satisfies (mechanical, zero behavior change — `kubernetes.NewForConfig` already returns
  a value assignable to the interface) so `kubernetes/fake.NewSimpleClientset()` becomes
  injectable, then (b) author the Job/Secret/SA/WIF manifest tests **fresh** as part of the
  move into `internal/k8sjob`, so the package reaches the §13 ≥90 gate honestly rather than
  by assuming pre-existing coverage. This is **not** executed in this STOP-gate check
  (scoped to verification only) — recorded as a mandatory expansion of step 3's scope for
  the implementation pass.
- **A4 (non-blocking, reinterpreted):** step 5's "tf run handler… engine constructed per
  run" target is satisfied by wrapping the **existing** `SingleRunWorker.ExecuteRun` — it
  already has the per-run shape an "Engine" would (per-run `Decryptor`-based DTO→Run
  mapping, per-run runToken-only `RunApi`, per-run working dir/log) — rather than by first
  building a not-yet-existing `Engine` type. The polling-only `Manager`+`Worker` (token
  protocol, `handleFetchRunError`) is what step 6 actually replaces with `dispatch.Loop`+
  `InProcess`; `SingleRunWorker` is reused by both the frozen k8s single-run path and the
  new in-process handler, which *strengthens* §16.7's "single-run could reuse the handler"
  note into "the handler already reuses `SingleRunWorker`" rather than requiring it as
  future work. §4.2's `Execute(ctx, ClaimedRun) error` signature is unaffected — it is an
  illustrative signature (P3) satisfied by an adapter over `SingleRunWorker`, not by an
  `Engine` literal.
- **A5 (non-blocking, location correction):** §5's package-fate table row for
  `decryption.go` ("already gone (plan 03 → `meshapi.DecryptRunDetails`)") is corrected —
  it has not moved. Step 3 instead moves `internal/controller/decryption.go` verbatim into
  `internal/k8sjob/decryption.go` (a same-shape, same-package-family move, exactly like
  every other file in §5's table), not a merge into `meshapi`. `NodeId` stays a `string`
  constructor parameter for this phase — introducing the named type is deferred as a
  non-blocking P8 nicety, not a phase-5 gate.
- **A6 (non-blocking, accepted debt):** the global `AppConfig`/`DiscoveredOidcIssuer`/
  `UseTestClient` state is read, not restructured, by every step in §11.1 (moves are
  "moved not changed" per §5) — de-globalizing it is out of phase 5's stated scope and was
  already out of phase 4's. Left as continuing, visible debt (flagged in uncertainties
  below given it is now a 3-phase pattern, not because it blocks this phase).
- **A8 (non-blocking, tracking-only):** the two new gate lines this phase adds (`dispatch`,
  `k8sjob`, §13) are unaffected by `crypto` remaining ungated; that is orthogonal debt
  inherited from phase 3, not this phase's to fix.
...
`Loop`+`InProcess`+tf handler against the **`meshapitest` server** (plan 03 §5.7 —
the real `net/http/httptest` meshfed mock at `internal/meshapitest`, seedable runs +
captured requests) so N simultaneous runs exercise the **real transport** (the
D6-preferred replacement for a hand-rolled fake `RoundTripper` in these integration
tests), with `MockedTfFacade` retained for the tf-tool side (the phase-1 style).
...
| 9 | **Acceptance + self-review gate.** local-dev-stack flow with the tf persona on the new loop (≥1 TERRAFORM + ≥1 MANUAL run); **N-concurrent smoke**: `maxConcurrentRuns=2`, two TERRAFORM runs verifiably overlapping (log timestamps); controller evidence: the A3 `kubernetes/fake` goldens remain the hermetic PR-gate proof (+ container smoke); the phase-7 opt-in kind controller e2e (plan 07 §5.4 — real `run-controller` in a kind cluster, coordinator = `meshapitest`, asserting a real k8s Job is created) is the evidence that closes the inherited plan 03 §12.3 "no in-repo controller e2e" gap; P1–P8 walk; PR lists flags (§16) + retargets (§11.2). | — | evidence in PR description (STOP-E) |
...
- **k8sjob:** A3 `kubernetes/fake` goldens unchanged — they stay the **hermetic PR-gate
  proof** of the k8s dispatch path; new tests only for the moved decryption placement
  (decrypt-failure ⇒ no report, transcript-empty pin from step 1) and config validation
  relocation. Real k8s-dispatch (a live Job actually created in a cluster) is **not** a
  phase-5 PR gate: it is the phase-7 opt-in kind controller e2e (plan 07 §5.4 — real
  `run-controller`, coordinator = `meshapitest`), which is the evidence that closes the
  inherited plan 03 §12.3 "no in-repo controller e2e" gap.
