...
// PATCH through the real meshapi.Client dialing this mock over real HTTP (the D6-preferred
// style, §5.7/§9) rather than a hand-rolled http.RoundTripper.
...
// one of the two D9-pinned "no run" statuses on the claim endpoint.
...
// Test_Claim_NoRunStatusOverride proves the D9 409-on-claim = no run branch is reachable
...
// by the real client (D9's "409-on-register = success" pin), driven from a seeded response.
...
// D9 pins: a live abort flag on a 200, and the already-aborted 409 {runAborted:true}.
