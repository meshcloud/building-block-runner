...
// is PENDING (P8: enums get a defined zero value) so an uninitialized ExecutionStatus never
...
const (
	...
	// never observes a stale IN_PROGRESS that only clears after its own long timeout (D9,
	// plan-05 H7 graceful-shutdown amendment). The runner-facing status endpoint accepts the
...
// stray value is the wrong failure mode here (the same move already made for Behavior, B12).
