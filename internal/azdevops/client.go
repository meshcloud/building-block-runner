...
// end). rootCAs trusts the same catrust.RootCAs() pool as meshapi.SharedHTTPClient, so this
// standalone client's outbound HTTPS is not left off the CUSTOM_CA_CERTS_PATH trust anchor
// (nil is a no-op: system trust only). Kept for cmd/azdevops + cmd/bbrunner/azdevops wiring
// and tests that want a non-retrying client; the default wiring (NewHandler's nil-HTTP
// fallback) instead reuses meshapi.SharedHTTPClient, the process-wide
// bound+retrying+redirect-policy client.
func NewHTTPClient(timeout time.Duration, rootCAs *x509.CertPool) *http.Client {
	if timeout <= 0 {
		timeout = adoHTTPTimeout
	}
	return httpclient.NoRedirectClient(timeout, httpclient.WithRootCAs(rootCAs))
}
