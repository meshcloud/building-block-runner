// Package azdevops is the AZURE_DEVOPS_PIPELINE run handler (PLAN_DETAIL_06C_azdevops.md):
...
// client_test.go) is the surviving pin for that behavior (§3, D6 Kotlin corollary; see
// CROSS_REPO_TODO.md for why no NEW Kotlin JUnit tests were added in this environment).
//
// Built on the 06A template (internal/manual): the unified report.Reporter (stateless,
// abort discarded), config.SingleRunMode/BlockRunnerCompat/ResolvePrivateKey,
// dispatch.StandaloneClaimClassifier, and the per-persona Dockerfile pattern. Package-local,
// not split into sibling packages (D11): the Poller/StatusUpdater/StatusMapper Kotlin
// classes dissolve into pure functions + one Handler method, not real package seams (§4).
package azdevops

// pipelineRun mirrors the Azure DevOps client/PipelineRun.kt DTO (§2.5/§4.2): package-local,
// not shared with any other persona package (a sibling persona package must not import
// another, umbrella §5.3 precedent). Result is a pointer because Kotlin's PipelineRunResult
// is a genuinely nullable field distinct from its own "unknown" enum member (U-P7's
...
// (U-P2): fallback triggers only on an absent key/pointer, never on an empty string value.
...
// U-P7's final-message rendering, which uses Kotlin's default toString(), not .value).
// Unlike a Kotlin enum, an unrecognized wire string is representable here rather than
// failing to parse -- the §4.5 tolerant-parse decision (an unknown state never blocks
...
// resultEnumNameOrNull renders U-P7's final-message result token: Kotlin's string-template
...
// them, and none has a valid empty-string wire value to collide with (P4's simple-nullable
// carve-out).
...
// parse, §2.5/§4.5). "" represents an absent/null state.
