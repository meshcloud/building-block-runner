...
// queues so poll_test.go can script a run's state across multiple GET calls (P-P3/P-P4).
type seqADO struct {
	*httptest.Server
...
// (the 2nd Now() call) read as already-expired relative to the 1st call's deadline (P-P1),
...
// dialing srv) and the given reporter factory/clock, mirroring the persona wiring.
func newTestHandler(reporters ReporterFactory, clock Clock) Handler {
	return NewHandler(Config{Uuid: testUuid}, HandlerDeps{
		Reporters: reporters,
...
func testCryptoForHandler(t *testing.T) *meshcrypto.MeshCertBasedCrypto {
	t.Helper()
...
func encryptForTestHandler(t *testing.T, crypto *meshcrypto.MeshCertBasedCrypto, plaintext string) string {
	t.Helper()
