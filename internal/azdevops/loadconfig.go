...
// so a runner type run with no config file and no env is effect-equivalent to the JVM
// default boot.
const (
	defaultUuid     = "a9786b14-ecfe-44dd-b04c-2bcfd326aa23"
	defaultApiUrl   = "http://localhost:8304" // the mux AZURE_DEVOPS_PIPELINE port
...
// the Kotlin-era `blockrunner:` compat block. A mounted Kotlin runner-config.yml
...
// (defaults < blockrunner: compat block-normalized-over-flat-keys < env). It fails fast
...
// overridden by VERSION / blockrunner.version when set.
func LoadConfig(log *slog.Logger, buildVersion string, singleRun bool) (Config, error) {
	...
	// configure the runner type); PrivateKey/PrivateKeyFile are read directly off the
	// block (BlockRunnerCompat.ApplyShared only normalizes the cross-type fields).
	...
	// Env bindings win last. Names are the literal shipped spellings; no Spring
	...
	// privateKeyFile > default path, falling back to the inline yaml privateKey)
...
// applyMaxConcurrentRuns honors RUNNER_MAX_CONCURRENT_RUNS (new, additive): a
