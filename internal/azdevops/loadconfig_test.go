...
func TestLoadConfig_Defaults_RequirePrivateKeyInPollingMode(t *testing.T) {
	...
	// not exist in the test environment -- LoadConfig must fail fast, not boot with an
...
// TestLoadConfig_ShippedContainerConfig proves the actual shared fit config file the
// Dockerfile bakes in (cmd/runner-config.yml) loads cleanly. The image no longer bakes a
// private key (keys are operator-supplied via RUNNER_PRIVATE_KEY_FILE / privateKeyFile /
// privateKey), so polling mode -- which requires a resolvable key -- must fail fast on the
// shipped file as-is; single-run mode has no such requirement and still resolves uuid/api
// (api.url falls back to the compiled-in defaultApiUrl, since the shared file drops url).
...
// blockrunner.debugMode key is inert but not silently dropped for the azdevops type
...
func TestConfig_Validate(t *testing.T) {
	...
	require.NoError(t, Config{
		BaseConfig: config.BaseConfig{Uuid: "u", Api: config.Api{Url: "x", Username: "a", Password: "b"}}, PrivateKey: "pem",
	}.Validate(false))
}
