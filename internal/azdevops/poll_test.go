...
// Test_Scenario_SyncPoll_AlreadyCompleted is P-P2: an already-COMPLETED trigger response
...
// Test_Scenario_SyncPoll_Happy is P-P3/U-P3-U-P7: one iteration inProgress->completed with
...
func Test_Scenario_SyncPoll_Happy(t *testing.T) {
	...
	// Iteration 2: run completed; "build" is COMPLETED again (re-sent, U-P5), "deploy" now
...
// Test_Scenario_SyncPoll_EmptyStages is U-P6: empty stage list falls back to the
// run-state-only update, sent unconditionally (no dedup on this path -- only the
// timeline-*failure* fallback dedups, §2.3).
...
// Test_Scenario_SyncPoll_Timeout is P-P1: a fake Clock already past the 30-min budget
...
// Test_Scenario_SyncPoll_Resilience is P-P4: a GET-run failure is retried (never fatal); a
...
// Test_Scenario_SyncPoll_ReportFailureEscalates is P-P5: a PATCH failure sending the final
...
// Test_Scenario_SyncPoll_CtxCancelReportsTerminal is the plan-05 H7 amendment (§4.4/§16.11):
// ctx cancellation mid-poll reports a TERMINAL status (ABORTED), never leaves the run
...
// FAILED if ABORTED is rejected" half of D9.
