...
// Test_RunSingleRun_AsyncCapturedWire verifies an async run JSON via RUN_JSON_FILE_PATH
...
// Test_RunSingleRun_MissingEnv covers the missing-env rung of the exit-code tail.
...
// Test_RunSingleRun_FileNotFound / Test_RunSingleRun_UnparsableFile are the sanctioned
// delta: Go exits non-zero on a pre-report fetch/parse failure where Kotlin
...
// Test_RunSingleRun_RegisterFailureExitsNonZero pins the register-failure rung (Kotlin
...
// itself performs the poll (unchanged from Kotlin) -- proven here with an
