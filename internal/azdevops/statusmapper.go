...
// This file is the Go twin of AzureDevOpsStatusMapper.kt (§2.4, S-P1-S-P6): pure functions,
...
// mapPipelineResult is S-P1: succeeded -> SUCCEEDED; failed, canceled, unknown, or absent
// (nil) -> FAILED.
...
// mapStateUserMessage is S-P3. state is a pointer purely to reproduce the Kotlin function's
// nullable parameter for the pin table (mapPipelineStateToUserMessage(state:
// PipelineRunState?)); pipelineRun.State itself is never absent in practice (§4.2).
...
// mapStageStatus is S-P4, including the else-holes pinned as-is (D13): completed+
...
// mapStageUserMessage is S-P5, including the else branch: completed+canceled/abandoned
...
// buildStageSystemMessage is S-P6: "Stage: <name>, State: <state|Unknown>[, Result:
