...
// report.Reporter (06A §4.3): each function returns exactly the changed/new steps for one
// Report call -- the reporter itself is stateless (umbrella §7.5), so all dedup bookkeeping
...
// PENDING status itself; DisplayName is what matters here for the registration body, C-P3).
...
// triggerSuccessUpdate is U-P1: run IN_PROGRESS, step SUCCEEDED, sync/async message variant
// (§2.1.4).
...
// stateOnlyUpdate is U-P6: run+step IN_PROGRESS, the mapped state message, `state.value`
// wire rendering (§2.3 empty-stage-list path -- sent unconditionally every call the caller
...
// stageBatchUpdate is U-P3/U-P4/U-P5: filters STAGE records with no parent, sorted by
// order; empty -> delegates to stateOnlyUpdate (§2.3); otherwise re-includes the trigger
// step first (SUCCEEDED, the no-polling-suffix message pair, distinct from
// triggerSuccessUpdate's, U-P4) plus every stage not yet reported OR currently COMPLETED
// (the one-way dedup, U-P5 -- COMPLETED stages are re-sent on every subsequent call).
...
// finalUpdate is U-P7: only the trigger step, status = the mapped result (can flip
// SUCCEEDED->FAILED at the step level), the enum-NAME rendering for state/result (distinct
// from the polling messages' wire-value rendering, §2.4/flag §16.4).
...
// failedUpdate is U-P8: run+step FAILED, the fixed user message, message is the
...
// (plan-05 H7 amendment, §4.4/§16.11): ABORTED first, FAILED as the fallback if the endpoint
