...
// TestNoStdlibLogInInternal enforces the single logging stack (D15):
// every package under internal/* logs through log/slog, never the stdlib "log" package.
//
// This Go test stands in for a depguard "log" deny rule. depguard's deny is
...
// so the invariant is enforced here instead. The run-log byte sink (internal/report) is
	...
	err := filepath.WalkDir(internalRoot, func(path string, d fs.DirEntry, err error) error {
		...
		for _, imp := range f.Imports {
			if imp.Path.Value == `"log"` {
				t.Errorf("%s imports the stdlib \"log\" package; use log/slog (single logging stack)", path)
			}
		}
		return nil
	})
