...
// K8sJobConfig holds the Kubernetes-specific configuration for KubernetesJobDispatcher: the
// dissolution target of the former internal/controller/config.go's k8s-facing fields
// -- namespace, job templates, tolerations, node selector and image
// pull secrets. Registration/crypto/api fields stay in the runner type (cmd/bbrunner) config,
...
//
// This default changed from 20 to 10 (a more conservative default for cluster job
// pressure); operators can still set any value via maxConcurrentJobs.
...
func (c K8sJobConfig) Validate() error {
	if c.Namespace == "" {
