...
// matrix, D7) that were never consumed by a Path/Env call or a ${VAR} interpolation
// during Load. Call it once, after every Path/Load/Env call for a persona's startup.
//
// A relaxed-binding holdover this loader does not recognize must surface as a hard
// startup error (P5) -- never as a runner that boots on wrong defaults and polls
// forever (D7).
