...
// Port is a resolved TCP port number for a management listener. The named type
...
// managementPortEnv is the primary env var every runner type checks first. It is
// additive, not a legacy alias -- a fresh var, so it carries no
// deprecation baggage of its own. Runner-type-specific legacy aliases (e.g. tf's PORT) are
...
// type, `ManagementPort(log, 8100, EnvAlias{Var: "PORT", Deprecated: true})` keeps
// every existing deployment's resolved port unchanged while introducing the new
// canonical var.
//
// A value that is set but not a valid, non-zero uint16 port is a startup-fatal
// misconfiguration: silently falling back to def would let a typo'd override boot
