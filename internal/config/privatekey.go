...
// runner types: env RUNNER_PRIVATE_KEY_FILE (non-blank)
...
// that exists but cannot be read is a hard error — a misconfigured key must fail
// fast, never silently fall through to an empty key.
//
// It is shared by the decrypting runner types and ships here with the template even
// though the manual type decrypts nothing; tf's own key handling is
