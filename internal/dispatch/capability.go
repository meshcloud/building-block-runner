...
// claimed run's type has neither an in-process handler nor a k8s job template (D5).
package dispatch
...
// ALL. The backend's BuildingBlockRunnerCapabilityType is a single-valued enum (D5,
...
// only (PLAN_DETAIL_05 §9): what a runner actually claims is decided server-side by the
...
// ALL. Config validation must fail fast (P5) on a typo'd or stale capability value at
