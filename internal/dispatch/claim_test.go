...
func TestRunClaimClient_Claim_ReturnsClaimedRun(t *testing.T) {
	...
	dto := &meshapi.Run{Metadata: meshapi.RunMetaDTO{Uuid: "run-1"}}
	srv.SeedRun(dto)
	...
	if run.Run.Metadata.Uuid != "run-1" {
		t.Errorf("expected Details to carry the fetched dto, got %+v", run.Run)
	}
	...
	var roundTripped meshapi.Run
	if err := json.Unmarshal(decoded, &roundTripped); err != nil {
		t.Fatalf("expected RawJson to decode back to a Run: %v", err)
	}
