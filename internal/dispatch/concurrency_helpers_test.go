...
// This file holds small shared fixtures for the phase-5 concurrency-hazard suite (plan
// PLAN_DETAIL_05_dispatcher.md §7, risk #4): one named test per hazard lives in its own
// sibling _test.go file; only the generic plumbing they all need lives here.
...
// captured requests/handler-recorded state, not on log output (H3 is the one exception and
// builds its own logger explicitly).
...
// tests can assert on the loop's claim cadence (H6).
