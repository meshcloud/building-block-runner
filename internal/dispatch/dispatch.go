...
// ClaimedRun wraps a meshapi.Run with its claim context. Sensitive values inside
// RawJson/Run are still encrypted -- decryption is placed inside the owning
// Dispatcher, not the loop, so each dispatch path keeps its own pinned decrypt-failure
// behavior.
type ClaimedRun struct {
	Id RunId
	// Type is resolved by Loop from Run before Dispatch is called (ToRunnerType of the
