...
// that are still running on shutdown (D9/§6/§7 H7). It is deliberately longer than a typical
...
// InProcess is the standalone-persona Dispatcher (D1/D5): it runs each claimed run in its own
// goroutine inside the same process, instead of handing it to a k8s Job. Which run types it
// can serve is fixed at LINK TIME by the handlers its binary wired in -- a fit persona binary
...
// message (§10.1) rather than being executed.
//
// Concurrency model (risk #4): the in-flight counter is incremented synchronously on the
...
// not-yet-counted run (H6). A completion also pokes Done() so the loop drains again
...
type InProcess struct {
	...
	// report a terminal status (never a stale IN_PROGRESS, D9/H7).
...
// construction, as is a nil handler -- so a constructed InProcess is always usable (P8). A
...
// returns a *UnhandledTypeError (D5 claim-and-fail-fast) WITHOUT spawning a goroutine or
...
func (d *InProcess) Dispatch(run ClaimedRun) error {
	...
	// exact, never racing a goroutine that has not yet counted itself (H6).
	d.mu.Lock()
	d.inFlight++
	d.mu.Unlock()
...
// ABORTED/FAILED status (never a stale IN_PROGRESS, D9/H7), then waits for them to return.
