...
// Test_InProcess_ConcurrentRuns_LogsAreIsolated pins the concurrency invariant:
// concurrent runs sharing one log stream (as every type's runs realistically do --
...
// (base.With("runId", ...)) exactly as the eventual tf/handler ports must: if a
