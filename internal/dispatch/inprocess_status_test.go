...
// Test_InProcess_ConcurrentRuns_StatusUpdatesDoNotInterleave pins concurrency hazard H4
// (plan-05 §7): concurrent runs must never let one run's status snapshot bleed into
// another's PATCH body -- the exact failure mode a shared/mutable status struct (the B6/B10
// pattern the high-level plan calls out) would produce. Each run sends several PATCHes
