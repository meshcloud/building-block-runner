...
// Test_InProcess_ConcurrentRuns_UseOwnRunTokens pins concurrency hazard H5 (plan-05 §7,
// risk #5): with the former single shared mutable run-token slot gone (§8), each run's
...
func Test_InProcess_ConcurrentRuns_UseOwnRunTokens(t *testing.T) {
	...
	handler := concurrencyTestHandlerFunc(func(ctx context.Context, run dispatch.ClaimedRun) error {
		...
		// (§8): built fresh per run, from run.Details.Spec.RunToken alone.
