...
// Test_InProcess_ConcurrentRuns_UseIsolatedWorkingDirs pins concurrency hazard H2 (plan-05
// §7): concurrent runs must never write into each other's working directory. The handler
// below stands in for the not-yet-landed tf run handler (plan §4.2/§4.3) and follows the
