...
// former controller-only pollingIntervalSeconds/maxConcurrentJobs pair so any runner type can
// drive the same drain loop.
type LoopConfig struct {
	...
	// tick" -- the run-controller type's behavior.
...
// LoopDeps are Loop's collaborators, all injected (no package-level mutable state).
type LoopDeps struct {
	...
	// the run-controller type's frozen policy.
	Classify ClaimClassifier
	Metrics  *MetricsCollector
	// Standalone is the optional per-runner-type meter for the two additive standalone-runner
	// counters the run_controller_* MetricsCollector has no equivalent of: unhandled run
	// type and at-capacity skip (the runner_runs_unhandled_total /
	// runner_at_capacity_skips_total series, structurally satisfied by *observability.RunMetrics).
	// nil for the run-controller type -- its run_controller_* series already covers
...
// StandaloneMetrics is the optional standalone-runner-type meter Loop drives for the two events
// the frozen run_controller_* collector does not model (unhandled-type fail-fast and the
// in-process at-capacity skip). Declared here (consumer-side) so *observability.RunMetrics
...
// Loop is the backend-agnostic claim/drain loop, generalized from the
...
type Loop struct {
	...
	// same fix already applied to internal/tf.Manager's identically-shaped field (a
...
// NewLoop constructs a Loop. A nil deps.Logger falls back to slog.Default()
...
const (
	...
	// FAILED (every dispatch failure is reported -- the former decrypt-failure
...
func (l *Loop) run() {
	l.logger.Info("dispatch loop running", "pollInterval", l.cfg.PollInterval)
...
func (l *Loop) drainRuns() {
	...
	if capacity <= 0 {
		l.logger.Info("at capacity, skipping run fetch this cycle", "maxConcurrent", l.cfg.MaxConcurrent)
...
func (l *Loop) processNextRun() processResult {
	...
	if err := l.deps.Dispatcher.Dispatch(run); err != nil {
		...
		// Every dispatch failure is now reported (never suppress silently). The
		...
		l.logger.Error("failed to dispatch run", "runId", run.Id, "error", err)
...
// claimed run's runToken: fail-fast fires before any handler owns the run.
func (l *Loop) reportRunFailure(runId RunId, errorMessage string) {
	if regErr := l.deps.StatusApi.RegisterSource(runId); regErr != nil {
		l.logger.Error("failed to register as status source", "runId", runId, "error", regErr)
		return
	}

	if statusErr := l.deps.StatusApi.UpdateRunStatus(runId, "FAILED", errorMessage, errorMessage); statusErr != nil {
		l.logger.Error("failed to report error back to meshfed", "runId", runId, "error", statusErr)
	}
}
