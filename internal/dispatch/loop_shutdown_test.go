...
// Test_Loop_ShutdownWaitsForInFlightRuns pins the concurrency invariant (the
...
//
// This drives the full type-shutdown sequence a real main would perform: dispatch.Loop
...
// handler that only reacts to context cancellation, exactly like a handler blocked
...
func Test_Loop_ShutdownWaitsForInFlightRuns(t *testing.T) {
	...
	// uphold the same contract they will (never leave a cancelled run's status
	...
	handler := concurrencyTestHandlerFunc(func(ctx context.Context, run dispatch.ClaimedRun) error {
		...
		// A "sync-polling" run: blocks until either it finishes on its own (it never
