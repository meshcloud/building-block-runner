...
// Test_Loop_WakeAndTickerRace pins concurrency hazard H8 (plan-05 §7): Loop's internal
// shutdown flag is read on every tick from the loop goroutine and written by Stop from
// whatever goroutine a persona's shutdown handling calls it from -- concurrent ticks and
...
// persona's Manager already fixed once (B6, sync/atomic). -race must stay clean here.
func Test_Loop_WakeAndTickerRace(t *testing.T) {
	...
	// can -- exactly the concurrent write/read pattern H8 targets.
