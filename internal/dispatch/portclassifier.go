...
// StandaloneClaimClassifier is the claim-error policy shared by the four phase-6
// Kotlin-port personas (manual/gitlab/azdevops/github), established by the manual template
// (06A §7.1, umbrella §7.3/§4 row 2). It reproduces the Kotlin catch-all: every claim
// failure is treated as "no run this tick" and retried on the next PollInterval — there is
// NO extra backoff (deliberately not the tf persona's 60s FAILED_WORKER_DELAY, which is
...
// for the next tick (LoopConfig.ClaimBackoff stays 0 for these personas).
