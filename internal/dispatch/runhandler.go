...
//
// Contract:
//   - run.RawJson / run.Run are still ENCRYPTED. The handler decrypts per run, inside its
...
//   - all run-scoped reporting MUST use the run's own runToken (run.Run.Spec.RunToken),
