...
// RunHandler executes exactly one claimed run to completion, in-process (D5). It is the
...
// the first implementation; the four Kotlin ports (phase 6) become further implementations,
// which is why this interface is deliberately minimal -- everything a handler needs travels
// in ClaimedRun plus its own constructor-injected deps (plan §4.3), so adding a handler
// never edits this package.
//
// Contract (plan §4.2/§8, risk #5):
...
//     B. This placement (handler-side, not loop-side) is also what preserves tf's D9-pinned
//     decrypt-failure UX -- a mismatch fails the run through the engine, not the loop (§16.2).
...
//     cross-authenticate (H5). The runner's main creds are for claiming only.
...
//     TERMINAL state -- report ABORTED (falling back to FAILED, never SUCCEEDED, D9) -- so the
