...
// appToken mints the GitHub App JWT (§4.2, §2.2): base64url(header) "." base64url(claims)
...
// (umbrella §4 row 16 / §16.13): the JWS is three base64url segments plus one
// rsa.SignPKCS1v15 call, and the code only ever *signs* (never verifies untrusted tokens),
// so a claims/validation framework buys nothing. Verification lives in the G-P1 test via
// rsa.VerifyPKCS1v15.
//
// The 10s iat skew and 300s exp window are FROZEN toward GitHub's App-auth acceptance (§13).
...
// parseAppPem reproduces the Kotlin PKCS#1 tolerance (§2.2/§4.2, G-P2): it is whitespace-
...
//     so we deliberately do not use pem.Decode — §4.2);
...
//
// Divergence recorded (flag §16.10): Kotlin rebuilds the key from modulus+privateExponent
