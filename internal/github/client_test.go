...
// Test_ClassifyDispatchResponse pins the 422 heuristic table (§4.3, keep-as-unit): status ×
...
// Test_Client_PermissionGate pins G-P7 at the client level: actions!=write ⇒ error carrying
...
func Test_Client_PermissionGate(t *testing.T) {
	...
	// A permission-gate failure is NOT an externalCallError (generic path, §2.6).
	if _, ok := asExternalCallError(err); ok {
		t.Error("permission-gate error should not be an externalCallError")
	}
}

// Test_Client_ExternalCallError pins the §2.6 MeshHttpException twin on a non-2xx installation
...
// Test_Client_DispatchContentType pins the sanctioned §16.2 delta: a single
