...
// find/poll waits (§5 row "Clock unification"). The Kotlin code mints the JWT from an
// unclocked Instant.now() while polling uses an injected clock (flag §16.9); the Go port
// deliberately routes both through one Clock so tests are fully deterministic.
