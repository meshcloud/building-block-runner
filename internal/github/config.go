...
// defaultMaxConcurrentRuns is the shipped default (plan 05 §5). It stays 1 for github
// because the workflow-run correlation window is heuristic and concurrent dispatches of the
// same workflow file can cross-track (flag §16.7): a higher default makes mis-association
// more likely, so operators opt into more concurrency explicitly.
const defaultMaxConcurrentRuns = 1

// Config is the github persona configuration (§6.1). GitHub coordinates (base URL, owner,
...
// phase-6 extras. The zero value is not usable in polling mode; build via LoadConfig and
// Validate (P8).
type Config struct {
	Uuid    string
	Version string
...
// Validate fails fast (P5) on an unusable polling-mode config. Unlike manual, a resolvable
