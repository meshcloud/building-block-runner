// Package github is the GITHUB_WORKFLOW run handler (PLAN_DETAIL_06D_github.md, D11): App
// JWT -> installation token -> workflow_dispatch, then async handover or sync run/job
// polling. It depends only on dispatch/meshapi/report/config + stdlib (§7 depguard).
package github

import "time"

// Step ids and display names are FROZEN meshStack-wire strings (§13): customer-visible in
...
// Workflow-dispatch input key names — FROZEN toward customer workflows (§2.4/§13): the
...
// GitHub API wire constants (§2.3, frozen §13).
const (
...
// Polling/find constants — constructor defaults (§4.1/§2.5), FROZEN correlation window §13.
const (
...
// behavior discriminators read off the run spec (§2.1.6, frozen §13).
const (
	behaviorApply   = "APPLY"
