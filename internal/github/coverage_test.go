...
// Test_Client_TransportError covers the transport-error branch (bad host). It must use an
// explicit non-retrying *http.Client: a nil client resolves to the retrying shared singleton,
// which would retry this GET's transport error and hang the test.
func Test_Client_TransportError(t *testing.T) {
	gc := newGithubClient("http://127.0.0.1:0", &http.Client{}, meshapi.SlogLogger(nil))
	if _, err := gc.installationId("t", "o", "r"); err == nil {
...
func Test_ReadRawInputs_Fallbacks(t *testing.T) {
	...
	out := decodeInputs("", details, testLog())
	if len(out) != 1 || out[0].Key != "k" {
		t.Fatalf("empty-raw fallback failed: %+v", out)
	}
	// invalid base64 ⇒ warn + details fallback.
	out = decodeInputs("!!!notbase64!!!", details, testLog())
	if len(out) != 1 {
		t.Fatalf("bad-base64 fallback failed: %+v", out)
	}
	// valid base64 but undecodable JSON ⇒ warn + details fallback.
	out = decodeInputs(encodeRawJSON("{not json"), details, testLog())
	if len(out) != 1 {
		t.Fatalf("undecodable-json fallback failed: %+v", out)
	}
	// nil details + empty raw ⇒ no inputs.
	out = decodeInputs("", nil, testLog())
	if len(out) != 0 {
		t.Fatalf("nil-details path failed: %+v", out)
	}
}
