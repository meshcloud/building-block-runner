...
// self href. The implementation carries realistic secret-bearing fields so the leak
// test is meaningful.
...
func parseRun(t *testing.T, raw string) *meshapi.Run {
	t.Helper()
...
// fixture field set + values (typed inputs preserved), and secret hygiene (no impl secrets leak; the
...
func Test_ModeA_Parity_And_Leak(t *testing.T) {
	inputs := `[
      ...
      {"key":"test4","value":4,"type":"INTEGER","isSensitive":false,"isEnvironment":true}]`
	raw := runJSON(inputs, false, "https://meshstack.example.com/api/meshobjects/meshbuildingblockruns/test")
	details := parseRun(t, raw)

	// Inputs already decrypted at the dispatch boundary ⇒ values unchanged, matching the
	// Kotlin byte fixture ("1","2","3",4).
	decoded := decodeInputs(b64OfRun(raw), details, slog.Default())
	...
	// No impl secret substrings appear anywhere in the payload.
	...
	// Parsed-JSON parity: assert the field set + values (null ≡ absent).
...
// Test_ModeA_NumberFidelity pins number fidelity: a large integer input survives as a number, not
...
// Test_DispatchInputs pins the Mode-B table: URL always, tokens iff present, endpoint
...
// Test_DecodeInputs_Passthrough pins that decodeInputs no longer decrypts: sensitive values
// arrive and leave verbatim (decryption happens at the dispatch boundary, internal/rundecrypt).
func Test_DecodeInputs_Passthrough(t *testing.T) {
	inputs := `[
	...
	out := decodeInputs(b64OfRun(raw), details, slog.Default())
	byKey := map[string]runInput{}
	for _, in := range out {
		byKey[in.Key] = in
	}
	assertEq(t, "s", byKey["s"].Value, "c1")
