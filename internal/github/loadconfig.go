...
// fallbacks (github-block-runner/src/main/resources/runner-config.yml:2-11) so a runner type run
...
const (
	...
	defaultApiUrl   = "http://localhost:8302" // the mux GITHUB_WORKFLOW port
...
// `blockrunner:` compat block. A mounted Kotlin runner-config.yml populates the
...
func LoadConfig(log *slog.Logger, buildVersion string, singleRun bool) (Config, error) {
	...
	// the runner type). debugMode is a manual-only key ⇒ warn-and-ignore here.
	...
	// Env bindings win last; literal shipped spellings, no relaxed-binding variants.
	loader.Env(log,
...
// applyMaxConcurrentRuns honors RUNNER_MAX_CONCURRENT_RUNS (additive). A negative
// value means unlimited; a non-numeric value is a hard startup error.
