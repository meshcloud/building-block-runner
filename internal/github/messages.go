...
// This file holds the FROZEN, UI-visible message strings (byte-identical to the Kotlin). The
...
// genericErrorMessage wraps a cause in the generic system message.
func genericErrorMessage(cause string) string {
	return genericErrorPrefix + cause
}

// systemMessageForError selects the message shape: an *externalCallError (the two
...
// triggerApiErrorMessage is the trigger-Error result message (:138-144).
...
// YAML snippets and the actions-register-source release link — FROZEN customer UX.
...
// triggerSuccessMessages are the IN_PROGRESS trigger-success step messages (:558-589):
