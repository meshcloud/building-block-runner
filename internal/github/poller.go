...
// pollWorkflow is the 06D-local sync poller (§4.5, D4 — package-local because it must first
...
//     completed-job re-report quirk preserved (G-P4);
//  3. reports the terminal update from the run conclusion.
//
// seenJobs is local per invocation (no handler state, P4). ctx cancellation (plan-05 H7,
// same as 06C): report a terminal ABORTED (fallback FAILED, never SUCCEEDED) then return
...
func (h Handler) pollWorkflow(ctx context.Context, reporter report.Reporter, gc *githubClient, impl meshapi.GithubImplementation, runID, workflow, token string, triggerTime time.Time) error {
	...
	for current.Status != runCompleted {
		...
		if err != nil {
			h.deps.Log.Warn("failed to get workflow run status, will retry", "run", runID, "err", err)
			continue
		}
		jobs, err := gc.workflowJobs(token, impl.Owner, impl.Repository, current.Id)
		if err != nil {
			h.deps.Log.Warn("failed to list workflow jobs, will retry", "run", runID, "err", err)
			continue
		}
		current = run
		if err := h.reportJobs(reporter, runID, jobs, seenJobs); err != nil {
			return err
		}
	}
	...
	if jobs, err := gc.workflowJobs(token, impl.Owner, impl.Repository, current.Id); err != nil {
		h.deps.Log.Warn("failed to get final job statuses", "run", runID, "err", err)
	} else if err := h.reportJobs(reporter, runID, jobs, seenJobs); err != nil {
		return err
	}
...
// reportJobs emits one IN_PROGRESS batch for the new-or-completed jobs (§2.5.3). New jobs
// are recorded in seenJobs; completed jobs are re-reported on every poll (G-P4 quirk,
// pinned as-is). The gh-trigger step is prepended ONLY in the first batch (when every
// reported job is new, i.e. seenJobs size == batch size after recording).
func (h Handler) reportJobs(reporter report.Reporter, runID string, jobs []workflowJob, seenJobs map[int64]bool) error {
	...
	_, err := reporter.Report(report.RunStatus{RunId: runID, Status: report.IN_PROGRESS, Steps: steps})
	return err
}

// jobStep maps one job to its step (§2.5.3): id gh-workflow-job-<id>, display "GitHub Job:
...
// reportFinal reports the terminal update from the run conclusion (§2.5.4): success⇒SUCCEEDED,
...
// reportAborted reports the graceful-shutdown terminal update (§4.5, plan-05 H7): ABORTED,
// falling back to FAILED if the endpoint rejects the ABORTED transition, NEVER SUCCEEDED —
// so the coordinator never sees a stale IN_PROGRESS. It then returns ctx.Err().
func (h Handler) reportAborted(ctx context.Context, reporter report.Reporter, runID string) error {
	...
	if err != nil {
		...
		if _, ferr := reporter.Report(report.RunStatus{
			RunId:  runID,
			Status: report.FAILED,
			Steps: []report.StepStatus{{
				Name:          StepId,
