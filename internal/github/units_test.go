...
func Test_NewHandler_Defaults(t *testing.T) {
	h := NewHandler(Config{}, HandlerDeps{})
	if h.deps.Clock == nil || h.deps.Log == nil || h.deps.HTTP == nil {
	...
	// default HTTP is the shared singleton (follows redirects, disabled per-request on the
	// dispatch trigger via meshapi.WithNoRedirect).
	if h.deps.HTTP.CheckRedirect == nil {
		t.Error("default HTTP client should carry the sentinel CheckRedirect")
	}
}
