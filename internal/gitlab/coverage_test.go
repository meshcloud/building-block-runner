...
// TestNewHandler_Defaults exercises NewHandler's nil-defaulting branches (a
...
func TestNewHandler_Defaults(t *testing.T) {
	...
	h := NewHandler(Config{BaseConfig: config.BaseConfig{Uuid: testUuid}}, HandlerDeps{Reporters: factoryFor(srv)})
...
func TestDecodeRawRunJson_Errors(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			run := dispatch.ClaimedRun{Id: dispatch.RunId(testUuid), Run: dto, RawJson: c.rawJson}
...
func TestDecodeImplementation_Errors(t *testing.T) {
	...
	t.Run("implementation is not a JSON object", func(t *testing.T) {
		...
		_, err := decodeImplementation(dispatch.ClaimedRun{Id: "r1", Run: dto})
		require.Error(t, err)
	})
...
// directly (unreachable through Execute, since meshapi.SanitizeRunObjectForHandover always
// re-marshals valid JSON on success).
