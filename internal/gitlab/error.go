...
// ExternalCallError is the MeshHttpException equivalent:
...
// package gitlab, with its first consumer -- the sibling type ports define their own
// identical-shaped type ("per-package typed error with the same fields"); a shared
// package would be ceremony for a 5-field struct until review decides otherwise.
type ExternalCallError struct {
	// UserMessage/SystemMessage classify the failure (rows A1-A4). They surface only
	...
	// pair here is diagnostic, not wire content.
	UserMessage   string
...
// classifyGitlabError reproduces GitLabClient.kt:69-107's four-way classification:
...
// garbage, GitLabClient.kt:101-107), this logs the actual parsed body (logs are not a
// contract). body is already known to parse as JSON at the call site, but this falls back
// to the raw bytes defensively rather than panic.
