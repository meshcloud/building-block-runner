...
// type run with no config file and no env is effect-equivalent to the JVM default boot.
const (
	defaultUuid     = "bfe76555-7a69-48e8-8cc0-8e02eb76fc22"
	defaultApiUrl   = "http://localhost:8303" // the mux GITLAB_PIPELINE port
	defaultUsername = "bb-api"
	defaultPassword = "guest"

	// defaultBaseConfigFile is the shared top-level base layer:
	// it holds the well-known dev private key common to gitlab/azdevops/github.
	// gitlab is the first port to actually wire a non-empty base layer -- manual
...
// fileConfig is the yaml surface the runner type loader decodes: the Go-native flat keys plus
// the Kotlin-era `blockrunner:` compat block. A mounted Kotlin
...
// block < env (the block sits above the flat keys within the file layer -- a mounted
// Kotlin-era file must fully configure the runner type). It fails fast
...
// pipeline trigger token). buildVersion is the ldflags build version, overridden by
...
func LoadConfig(log *slog.Logger, buildVersion string, singleRun bool) (Config, error) {
	...
	// configures the runner type); PrivateKey/PrivateKeyFile are the gitlab-specific keys read
	// directly off the block (manual ignores them).
	...
	// Env bindings win last. Names are the literal shipped spellings; no
...
// applyMaxConcurrentRuns honors RUNNER_MAX_CONCURRENT_RUNS: a negative value
// means unlimited (bounded per-cycle by the loop's backstop). A non-numeric value is a
// hard startup error, never a silent fallback.
