...
// DiscoverOIDCIssuer (§13); NewKubernetesJobDispatcherWithClient (kubernetes.go) is the
// hermetically-testable half every test in this package actually exercises.
func NewKubernetesJobDispatcher(cfg Config, runnerUuid, apiUrl string, crypto *meshcrypto.MeshCertBasedCrypto, metrics JobMetrics, logger *slog.Logger) (*KubernetesJobDispatcher, error) {
...
// stays per-file honest (PLAN_DETAIL_05_dispatcher.md §13). Everything else in this package
...
func DiscoverOIDCIssuer(logger *slog.Logger) string {
	...
	if resp.StatusCode != http.StatusOK {
		logger.Warn("OIDC configuration endpoint returned non-OK status", "status_code", resp.StatusCode)
		return ""
	}
