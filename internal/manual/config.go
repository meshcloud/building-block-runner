...
// runner types: a modest throughput improvement over the former serial
// single-worker cadence, overridable via RUNNER_MAX_CONCURRENT_RUNS.
const defaultMaxConcurrentRuns = 3

// Config is the manual type's configuration: the shared meshfed API/auth section plus
// the manual-only extras. The zero value is not usable in polling mode — construct it
// through the runner type loader (cmd/manual) and Validate before use.
type Config struct {
