...
func TestExecute_RegisterFailureReturnsError(t *testing.T) {
	...
	h := NewHandler(Config{BaseConfig: config.BaseConfig{Uuid: testUuid}}, HandlerDeps{Reporters: factoryFor(srv), Log: testLog()})
...
func TestDecodeInputs_Fallbacks(t *testing.T) {
	details := &meshapi.Run{Metadata: meshapi.RunMetaDTO{Uuid: testUuid}}
	...
	for _, raw := range []string{"", "!!!not-base64!!!"} {
		srv := meshapitest.NewServer(t)
		run := dispatch.ClaimedRun{Id: dispatch.RunId(testUuid), Run: details, RawJson: raw}
		h := NewHandler(Config{BaseConfig: config.BaseConfig{Uuid: testUuid}}, HandlerDeps{Reporters: factoryFor(srv), Log: testLog()})
		require.NoError(t, h.Execute(context.Background(), run))
