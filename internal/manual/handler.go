...
// — it is injectable via Clock so tests run instantly.
...
// injected seam: cmd/manual wires the real meshapi-backed factory, tests wire one
...
// meshapi.Decryptor: the manual runner never decrypts — the template names one for every
// handler, and the other runner types add it, so this omission is a manual-specific narrowing,
...
// Handler is the MANUAL run handler (value type). It satisfies dispatch.RunHandler.
type Handler struct {
	cfg  Config
	deps HandlerDeps
}
...
// defaults so a minimally-wired handler is always usable.
...
// update sequence (debugMode). It follows the RunHandler contract: a
// register/report transport failure returns a non-nil error (infrastructure failure — the
// run stays unreported exactly as in Kotlin); manual never reports run-level FAILED
// because no execution can fail. manual has no poll loop to interrupt, so it honors a
// backend-requested runAborted (T1) by reporting terminal ABORTED instead of the
// otherwise-due SUCCEEDED/IN_PROGRESS update, rather than by cancelling a context.
func (h Handler) Execute(ctx context.Context, run dispatch.ClaimedRun) error {
	...
	// Register exactly the "manual" step (Kotlin registerAsSource in processBlock).
...
func (h Handler) executeEcho(reporter report.Reporter, runId string, inputs []meshapi.BuildingBlockInputSpecDTO, log *slog.Logger) error {
	...
	abort, err := reporter.Report(status)
	if err != nil {
		return err
	}
	if abort {
...
// reportAborted sends the single terminal ABORTED update for a run the backend has flagged
// via runAborted (T1). manual has no in-flight work to cancel, so this is the entire abort
// reaction: one report replacing whatever status was about to go out.
...
// sensitivity preserved, last-wins on duplicate keys (Kotlin associateBy).
...
// back byte-faithfully). This is a handler-visible template requirement recorded
// for gitlab/github, which embed run JSON in outbound payloads. When RawJson is
