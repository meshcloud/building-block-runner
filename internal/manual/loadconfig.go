...
const (
	...
	defaultApiUrl   = "http://localhost:8301" // the mux MANUAL port
...
// on an unconsumed BLOCKRUNNER_*-prefixed env var (a Spring relaxed-binding holdover)
// and — in polling mode — on an unusable config. buildVersion is the ldflags build
// version, overridden by VERSION / blockrunner.version when set.
func LoadConfig(log *slog.Logger, buildVersion string, singleRun bool) (Config, error) {
	...
	// the runner type); DebugMode is the manual-only key read directly off the block.
...
// applyMaxConcurrentRuns honors RUNNER_MAX_CONCURRENT_RUNS (new, additive):
