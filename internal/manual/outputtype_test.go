...
// Test_ToOutputType pins the 8-row input→output type table (M-P: keep-as-unit — a pure
// mapping with real decision surface, umbrella §5.2) plus the Go-only unknown-type row: an
// unknown value maps to itself and reports known=false so the handler warns rather than
// failing the run (§4.2/§16.5).
