...
// FetchRun's non-2xx paths must surface an HttpError whose Is* classify the frozen
...
// The 128MiB artifact stream is never routed through the body logger: only
...
func TestRunClient_DownloadArtifact_BodyNotLogged(t *testing.T) {
	...
	out := buf.String()
	assert.Contains(t, out, "meshapi response")
	assert.Contains(t, out, fmt.Sprintf("contentLength=%d", len(payload)))
