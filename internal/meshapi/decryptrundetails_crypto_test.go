...
// These round-trip tests migrated with DecryptRunDetails from internal/k8sjob. They reuse
// this package's testCryptoPair (a real CertDecryptor plus
...
// runDetailsWithImpl builds a minimal Run carrying implRaw as its implementation
...
func TestDecryptRunDetails_SensitiveInputs(t *testing.T) {
	...
	inputs := []BuildingBlockInputSpecDTO{
		{Key: "password", Value: ciphertext, Type: "STRING", IsSensitive: true},
...
func TestDecryptRunDetails_SensitiveInputDecryptFailure_IsAnError(t *testing.T) {
	dec, _ := testCryptoPair(t)
	inputs := []BuildingBlockInputSpecDTO{
		{Key: "password", Value: "not-actually-encrypted", Type: "STRING", IsSensitive: true},
	}
	impl := TerraformImplementation{Type: "TERRAFORM"}
	implRaw, err := json.Marshal(impl)
	if err != nil {
...
// TestDecryptRunDetails_UnsupportedSensitiveInputTypeFails pins the breaking behavior change
// for the k8s Secret-handover path: a sensitive building-block input whose declared type is
// not STRING/CODE/FILE (here BOOLEAN) now fails the run with an UnsupportedSensitiveTypeError,
// instead of the former decrypt-if-string / leave-otherwise rule. The per-implementation
// secret (SSH key / appPem / trigger token / PAT) is unaffected by this policy.
func TestDecryptRunDetails_UnsupportedSensitiveInputTypeFails(t *testing.T) {
	dec, _ := testCryptoPair(t)
	inputs := []BuildingBlockInputSpecDTO{
		{Key: "flag", Value: "ENC(x)", Type: "BOOLEAN", IsSensitive: true},
	}
	implRaw, err := json.Marshal(TerraformImplementation{Type: "TERRAFORM"})
	if err != nil {
		t.Fatalf("failed to marshal implementation: %v", err)
	}
	if _, err = DecryptRunDetails(runDetailsWithImpl(t, implRaw, inputs), dec); err == nil {
		t.Fatal("expected an error for a sensitive BOOLEAN input")
	} else {
		var uerr secret.UnsupportedSensitiveTypeError
		if !errors.As(err, &uerr) {
			t.Fatalf("expected UnsupportedSensitiveTypeError, got %v", err)
		}
	}
}
...
func TestDecryptRunDetails_ImplementationTypeParseFailure_IsAnError(t *testing.T) {
	...
	// Run's json.RawMessage field untouched) but is the wrong shape for
...
func decodeResult(t *testing.T, result string) Run {
	t.Helper()
