...
// Run represents the full structure of a building block run returned by the meshfed API.
type Run struct {
...
// RunInfo contains key information extracted from a Run.
type RunInfo struct {
	Uuid                             string
...
func (r *Run) GetRunInfo() RunInfo {
	return RunInfo{
		Uuid:                             r.Metadata.Uuid,
		WorkspaceIdentifier:              r.Spec.BuildingBlock.Spec.WorkspaceIdentifier,
		BuildingBlockDefinitionUuid:      r.Spec.Definition.Uuid,
		BuildingBlockDefinitionWorkspace: r.Spec.Definition.Spec.WorkspaceIdentifier,
	}
}

// ParseRunDetails parses run JSON bytes into a Run.
func ParseRunDetails(data []byte) (*Run, error) {
	var run Run
