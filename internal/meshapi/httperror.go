...
// registered semantics, D9) and surface the server's message.
//
// It replaces the former status-only StatusError and is shaped/named identically to the
// terraform-provider-meshstack client's HttpError so the future shared SDK merge (D3, §7)
...
// signal (D9); on the runner-registration PUT it means "create the runner via the UI".
...
// on the claim POST it means "no run available" (both frozen, D9).
