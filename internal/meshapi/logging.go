...
// slog-shaped) so the future shared meshstack-go-sdk merge has no logging-seam
...
// gating via LOG_LEVEL happens in the slog handler — no separate Enabled() check).
...
// Every other header (and every body) is logged in full at Debug.
...
// routed through it, so Debug logging cannot exhaust memory on a large artifact.
//
// The body is rendered verbatim: unlike the Authorization header, bodies are NOT redacted,
// so any secret carried in a payload (e.g. a CI trigger token or a decrypted sensitive
// input, once the external-CI clients route through this seam) is written to the log at
// Debug. This is an accepted trade-off — Debug is an opt-in operator-level knob — not a
// defect; an operator enabling Debug is choosing to log request/response bodies in full.
