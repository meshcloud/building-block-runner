// Package meshapitest is the shared meshfed-API mock server: a real
// net/http/httptest.Server exposing the frozen runner-facing endpoints
// (claim POST, register-source POST, status PATCH, artifact download GET) with seedable
// runs and captured requests. It prefers this real-HTTP style over a hand-rolled
// http.RoundTripper because it exercises the client's actual transport (headers, retry,
// media types) end to end; every runner-type's integration suite (the client
// test, the concurrency suite, per-type tests, the opt-in
// controller e2e) dials the same mock instead of hand-rolling its own fake.
//
// Test-only helper: not part of the coverage gate and not itself a wire contract —
...
// the runAborted response body the real endpoint uses to carry the abort flag —
...
// registration has exactly one frozen shape (RegistrationDTO).
...
// shapes exist on this endpoint (StatusUpdateDTO / RunStatusUpdateDTO) and the mock
...
// empty (default 404). Both 404 and 409 are pinned as "no run" on the claim endpoint, so
...
func (s *Server) SeedRun(run *meshapi.Run) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// http.StatusConflict to exercise the "409-on-register = success" pin. Once the queue
...
// tests drive the abort flag and the already-aborted 409 {runAborted:true} case.
