...
// RunMetrics is the generic standalone-runner instrumentation: runs claimed,
// succeeded, failed, run duration, and poll errors, each labeled by runner_uuid.
// Every type that lacks its own equivalent metrics wires this in (the run-controller
// type does not -- its run_controller_* series already covers the same events,
// making a duplicate runner_* series dashboard noise).
//
// RunMetrics structurally satisfies whatever small consumer-side "meter" interface a
// polling loop declares for itself (e.g. internal/tf.Meter); this package does not
// import a runner type package to enforce that, keeping the dependency direction adapters-do-
...
// run available" nor a benign transport quirk (see internal/tf's NewClaimClassifier).
func (m *RunMetrics) PollError() {
	m.pollErrors.WithLabelValues(m.runnerUuid).Inc()
}

// RunUnhandled records a claimed run whose type this runner had no handler for (claim-
// and-fail-fast). It is deliberately NOT a runner_runs_failed_total
// (that series means "executed and failed").
...
// run_controller_jobs_at_capacity_skips_total.
