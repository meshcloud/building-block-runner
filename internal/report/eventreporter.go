...
// needs, declared on the consumer side (P3) so the reporter is fakeable without HTTP and
...
// (HttpBlockRunClient.registerAsSource → MeshBuildingBlockRun defaults, 06A C-P3).
const pendingStatus = "PENDING"

// eventReporter is the event-driven implementation of the unified Reporter port used by
// the four phase-6 Kotlin-port runners (umbrella §7.5): Register once, then Report on
...
// speculative (P3). Each Report sends exactly the steps present in the passed RunStatus.
...
// underneath, built by the persona per run). A nil log falls back to slog.Default().
...
// entry, each PENDING (the frozen register body, C-P3). A 409 (already registered) is
// treated as success inside meshapi.RunClient (D9), so it surfaces here as a nil error.
...
// and returned so tf can honor it, but the four phase-6 runners discard it (umbrella
// §7.5). A malformed/empty response body is not an error — the abort flag simply reads
...
// toSourceUpdate maps a RunStatus onto the lean SourceUpdateDTO wire shape (§7.4). Message
// pointers are dereferenced to omitempty strings (null ≡ absent, §16.4).
