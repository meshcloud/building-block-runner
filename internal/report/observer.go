...
// using the Progress+Observer 10s ticker (D4); the four phase-6 ports call Reporter.Report on
...
// those changed steps, honoring the D4 "Report transmits only the changed/new steps" contract.
...
// handing over to an external pipeline is not itself the run's true completion (D9) — unless
// the context was already cancelled by an abort, in which case nobody wants a final update for
// a run that was just told to stop (D9's "cancelled ctx => no final update" pin).
type Observer struct {
	// Interval between status ticks. D9 pins this at 10s for tf.
	...
	// IN_PROGRESS (D9) because execution has only been handed off, not completed.
	Async bool
...
//
// This takes one more parameter than the high-level plan's illustrative 3-arg sketch
