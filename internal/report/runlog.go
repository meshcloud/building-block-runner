...
// plus a size field tracked by the caller (P8) — this generalizes the tf-only logwrap
...
// Deliberately returns an error rather than the plan's illustrative no-error signature: file
// I/O can fail (P5 — fail fast, never swallow), and every existing caller of the tfrun
