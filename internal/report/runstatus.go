...
// level (the domain shape must not depend on the wire DTO it is later mapped to).
type Output struct {
	Value     any
	Type      string
	Sensitive bool
...
// every runner's Reporter (tf and the four ports) is built around. Steps is a value
// slice (bugfix carried forward): the work goroutine only ever replaces it
...
// StepStatus fields are value/pointer-reassigned, never mutated in place, so a value
