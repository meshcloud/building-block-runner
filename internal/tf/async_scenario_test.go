...
// Async runs. Async terraform runs hand
// execution over to an external pipeline, so they collapse to a single "trigger"/"Prepare Run" step
// and report a final report.IN_PROGRESS (not report.SUCCEEDED) once the internal run succeeds, since
...
// report.FAILED (the report.IN_PROGRESS mapping is SUCCEEDED-only).
...
func (suite *WorkerTestSuite) Test_AsyncApply_SingleTriggerStep_FinalInProgress() {
	...
	suite.Equal(report.IN_PROGRESS.String(), *final.Status, "async success maps SUCCEEDED->IN_PROGRESS")
}
...
func (suite *WorkerTestSuite) Test_AsyncDestroy_AppliesBeforeDestroy() {
	...
	suite.Equal(report.IN_PROGRESS.String(), *final.Status)
}
...
func (suite *WorkerTestSuite) Test_AsyncApply_TfFailure_FinalFailed() {
	...
	final := decodeUpdate(suite.T(), (*updateCalls)[len(*updateCalls)-1])
	suite.Equal(report.FAILED.String(), *final.Status, "async failure is not remapped to IN_PROGRESS")
}

// collectUpdatesWorker mirrors the Handler suite's update collection for the polling suite.
