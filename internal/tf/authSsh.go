...
type SshAuth struct {
	...
	certStr        string
	prepDir        string
	// skipHostKeyValidation is threaded from the runner config in place of the
...
// unwrapCert returns the SSH private key: plaintext by the time it reaches here, decrypted at
// the claim boundary (rundecrypt.Wrap / the controller) rather than by this auth method.
func (sshAuth *SshAuth) unwrapCert() string {
	return sshAuth.certStr
}
...
func (sshAuth *SshAuth) toTransport(url string, log *logwrap) (transport.AuthMethod, error) {
	...
	//
	// Host keys are resolved only from explicit sources -- the configured knownHostEntry, then
	// SSH_KNOWN_HOSTS, then ~/.ssh/known_hosts (see combinedKnownHostsKeyCallback). We deliberately
	// do NOT auto-discover/scan the server's key: that would be trust-on-first-use (silently trust
	// whatever key the host first presents), defeating host-key verification. Keeping resolution
	// explicit is the intended posture; operators without a match must configure one.
