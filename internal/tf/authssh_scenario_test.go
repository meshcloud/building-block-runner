...
// Hermetic seam tests on the auth
// implementations (SshAuth/NoAuth). These are seam tests on the `auth` interface (a real
// consumer-side seam, gitsource.go), acceptable because a planned refactor moves this file intact into
// an adapter package. All key material is generated in-process (crypto/ed25519 + x/crypto/ssh)
...
func Test_SshAuth_UnwrapCert(t *testing.T) {
	// The cert arrives plaintext -- decryption now happens once at the claim boundary
	// (rundecrypt.Wrap / the controller), never inside SshAuth. Genuine decrypt/error cases
	// are covered by the boundary tests (internal/rundecrypt, meshapi.DecryptRunDetails).
	sshAuth := &SshAuth{certStr: "already-decrypted"}
	assert.Equal(t, "already-decrypted", sshAuth.unwrapCert())
}
