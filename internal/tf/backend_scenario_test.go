...
// plus the two matrix rows that live in
// the same code path: the env whitelist (cleanSystemEnv) and the init-retry behavior
...
// to end (real GitSource + hermetic local-repo clone) rather than the Worker matrix,
// because the backend/env logic has no HTTP-transport-observable surface of its own — the
// Worker matrix rows already prove the surrounding wiring.
//
// HINT_INIT_FAILED's APPLY/DETECT/DESTROY asymmetry is pinned in bug_inventory_test.go,
// not duplicated here.
...
// local git repo; it exposes everything the assertions need to inspect: the run's final status,
...
// runBackendFallbackScenario builds a real Run (hermetic local-repo GitSource) and drives a
// full TfApplyCommand.execute() against it, capturing the env map handed to TfFacade.SetEnv via the
// MockedTfFacade's setEnvFunc hook (mockedtffacade.go — test-infra extension).
func runBackendFallbackScenario(t *testing.T, repoFiles map[string]string, useMeshBackendFallback bool, runToken, meshstackBaseUrl string) *backendFallbackFixture {
	...
	cfg := TfRunnerConfig{
		BaseConfig:           config.BaseConfig{Uuid: "backend-fallback-scenario"},
		InitTimeoutMins:      1,
		WsTimeoutMins:        1,
	...
	opts := []runDetailsOption{withRepo(repo.Path, ""), withRunToken(runToken)}
	if useMeshBackendFallback {
		opts = append(opts, withMeshHttpBackendFallback())
	}
	dto := runDetailsDTO(opts...)
	dto.Spec.BuildingBlock.Uuid = "bb-backend"
	dto.Spec.BuildingBlock.Spec.DisplayName = "backend-fallback-test"
	dto.Spec.BuildingBlock.Spec.WorkspaceIdentifier = "ws-backend"
	dto.Links.MeshstackBaseUrl.Href = meshstackBaseUrl
	...
	runContextInfo, err := initRunContextInfo(dto, behavior, impl, "", slog.New(slog.NewTextHandler(io.Discard, nil)), wd)
	require.NoError(t, err)
...
func Test_BackendFallback_UseCaseMatrix(t *testing.T) {
	...
	t.Run("fallback on, no backend block in sources: writes http backend file, disables workspaces, sets TF_HTTP_* env", func(t *testing.T) {
		...
		require.Equal(t, report.SUCCEEDED, f.runContextInfo.runStatus.Status)
