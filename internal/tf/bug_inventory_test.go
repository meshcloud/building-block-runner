...
// bug_inventory_test.go pins the D13 bug inventory: each entry below asserts *correct*
// behavior together with the matching production fix. This file is deliberately
// self-contained (its own TfFacade test double, its own tfcmd/execute harness) so it stays
// disjoint from the other test files.
//
// Not pinned here, by design:
//   - manager.go shutdownCalled and runcontextinfo.go reportStatus shallow copy are
//     genuine data races. D13 exempts them from pin-verbatim: they were fixed
//     structurally (atomic.Bool / deep-copy) and are guarded by `-race` (on in CI/
//     Taskfile), not by a functional pin here.
//   - tfbinaries.go installTofuBinaries uses context.Background(): lives in a file excluded
//     from the coverage gate and needs a live network download to reach; inventory-only.
//   - gitsource.go nil-deref logging `*g.path` when it is nil is covered by a unit test in
//     gitsource_test.go, not here.
//   - main.go single-run failure only logged, process exits 0: lives in `package main`,
//     outside the `tfrun` gate; fixed in tf-block-runner/main.go.
...
// (formerly AppConfig.WsTimeoutMins/InitTimeoutMins); set positive so the derived
...
// --- fixed: selectWorkspace propagates the WorkspaceSelect error ---------------------------
...
// --- fixed: selectWorkspace returns the real matched workspace name -------------------------
...
// --- fixed: deleteWorkspaceIfNeeded returns after a selectWorkspace error --------------------
...
// --- fixed: plainInit's retry pause is a full second, as the log message promises ----------
...
// one lands with a future engine/ports refactor), so this asserts a real ~1s sleep rather than a fake clock.
...
// --- fixed: NewLogWrap returns (nil, error) on file-open failure ----------------------------
//
// The B5 sensitive-non-decryptable-type guard (Variable.decryptIfSensitive) moved to the claim
// boundary along with all decryption (meshapi.DecryptRunDetails / internal/secret); it is now
// pinned there, not in this package.
...
func Test_BugInventory_B7_InitRunContextInfoPropagatesLogWrapError(t *testing.T) {
	dto := runDetailsDTO()
	dto.Metadata.Uuid = "bug-inv-b7"
	dto.Spec.BuildingBlock.Uuid = "bb-7"
	dto.Spec.BuildingBlock.Spec.WorkspaceIdentifier = "_"

	// wd's "logs" subdir is deliberately not created, so the outFile path
	// ("<wd>/logs/logs-<runId>.txt") cannot be opened.
	wd := t.TempDir()
	rci, err := initRunContextInfo(dto, APPLY, meshapi.TerraformImplementation{}, "", slog.New(slog.NewTextHandler(io.Discard, nil)), wd)

	assert.Nil(t, rci)
	require.Error(t, err, "fixed: the log-file-open error now surfaces instead of a nil-logwrap RunContextInfo")
}

// --- fixed: Behavior.str()'s default branch no longer calls log.Fatalf --------------------
...
// DetermineBehavior contract pinned unchanged by the fix: it is still the parser callers use
...
// --- HINT_INIT_FAILED is printed for DETECT/DESTROY but not APPLY --------------------------

// makeBugInventoryRun builds a minimal, real (hermetic, local-filesystem) Run whose GitSource
// clones the given local repo — reusing the makeLocalGitRepo fixture rather than
...
func makeBugInventoryRun(repo *localGitRepo, behavior Behavior) *meshapi.Run {
	dto := runDetailsDTO(withBehavior(behavior.str()), withRepo(repo.Path, ""))
	dto.Metadata.Uuid = "bug-inv-b13-" + behavior.str()
	dto.Spec.BuildingBlock.Uuid = "bb-13"
	dto.Spec.BuildingBlock.Spec.WorkspaceIdentifier = "_"
	return dto
}
...
func runToInitFailure(t *testing.T, behavior Behavior) string {
	...
	dto := makeBugInventoryRun(repo, behavior)

	parsedBehavior, err := BehaviorFor(dto)
	require.NoError(t, err)
	impl, err := ParseTerraformImplementation(dto)
	require.NoError(t, err)
	...
	runContextInfo, err := initRunContextInfo(dto, parsedBehavior, impl, "", slog.New(slog.NewTextHandler(io.Discard, nil)), wd)
	require.NoError(t, err)
	...
	params := &TfCmdParams{
		...
		suggestedWorkspace: toWorkspaceStr(dto),
		vars:               vars,
		source:             source,
		runMode:            parsedBehavior.str(),
		initTimeout:        1 * time.Minute,
	}
