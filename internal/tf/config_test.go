...
func TestApplyEnvVars(t *testing.T) {
	...
	t.Run("applies RUNNER_SHUTDOWN_GRACE from environment", func(t *testing.T) {
	...
	t.Run("ignores an invalid RUNNER_SHUTDOWN_GRACE", func(t *testing.T) {
		t.Setenv(envShutdownGrace, "not-a-number")
		cfg := TfRunnerConfig{ShutdownGraceSeconds: 42}
		applyEnvVars(&cfg, nopLogger)
		require.Equal(t, 42, cfg.ShutdownGraceSeconds)
	})
