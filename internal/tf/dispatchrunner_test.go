...
// via observability.NewRegistry (so this tf test never has to import prometheus directly -- tf-test
// depguard forbids it).
func meterAndMetrics(uuid string) (*observability.RunMetrics, *dispatch.MetricsCollector) {
	reg := observability.NewRegistry()
	return observability.NewRunMetrics(reg, uuid), dispatch.NewMetricsCollectorWithRegistry(reg)
}
...
// idle poll (the old NORUN_WORKER_DELAY, wired into LoopConfig.PollInterval) and a 60s back-off
// after a claim-fetch error (the old FAILED_WORKER_DELAY, wired into LoopConfig.ClaimBackoff).
// The third leg -- immediate re-claim once a run completes -- is provided by the generic loop via
// InProcess.Done() (dispatch.InProcess signals completion so the loop drains again at once rather
// than waiting a full PollInterval), and is exercised in the dispatch package's capacity suite.
