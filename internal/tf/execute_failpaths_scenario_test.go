...
// CP13 continued (PLAN_DETAIL_01_tf_characterization_tests.md §9): the remaining execute() failure
...
func (suite *WorkerTestSuite) Test_Apply_MalformedTerraformConfig_StillSucceeds() {
	...
	final := decodeUpdate(suite.T(), (*updateCalls)[len(*updateCalls)-1])
	suite.Equal(SUCCEEDED.str(), *final.Status, "a malformed config only skips type-mismatch detection, it does not fail the run")
}
