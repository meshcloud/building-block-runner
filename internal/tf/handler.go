...
// claimed DTO -- already decrypted at the claim boundary (rundecrypt.Wrap) -- to an internal
// Run, builds a run-scoped RunApi authenticated with the run's own runToken, and drives the
...
func (h Handler) Execute(ctx context.Context, cr dispatch.ClaimedRun) error {
	...
	run, err := RunDTOToInternal(cr.Details)
	if err != nil {
		return err
	}
	// Hand the pre-run script the decrypted meshfed run object verbatim (cr.RawJson is the
	// base64 of the decrypted claim bytes on both the polling and single-run paths), rather
	// than a re-serialization of the typed DTO that would silently drop any field the DTO
	// does not model.
	run.RunJsonBase64 = cr.RawJson
	...
	api := h.newRunApi(cr.Details.Spec.RunToken)
	...
	return worker.tfExecution(ctx, run)
}
