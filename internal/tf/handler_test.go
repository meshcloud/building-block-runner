...
func Test_NewHandler_FillsDefaults(t *testing.T) {
	// A minimally-wired handler (nil Meter/Log/NewRunApi) must be usable (P8): every optional
	// dep falls back to a working default.
	h := NewHandler(HandlerConfig{WorkingDir: "/tmp", TfCommandTimeoutMins: 5}, HandlerDeps{})
	if h.meter == nil {
