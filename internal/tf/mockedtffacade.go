...
type MockedTfFacade struct {
	...
	// context is no longer smuggled through context.Value.
	workingDir string

	// setEnvFunc/workspace*Func are configurable seams (test-infra only) so scenario tests can
	// observe the subprocess env (cleanSystemEnv/TF_HTTP_* pins) and drive the workspace
	// select/create/delete naming logic without adding a production seam. initMockFuncs
