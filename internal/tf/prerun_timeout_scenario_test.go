...
// Pre-run script + failure UX at use-case
// level. The pre-run script contract: the script runs with a $MESHSTACK_USER_MESSAGE file whose
...
func (suite *WorkerTestSuite) Test_PreRunScript_Success_SetsUserMessage() {
	...
	suite.Equal(report.SUCCEEDED.String(), *final.Status)
