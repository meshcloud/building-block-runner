...
// The measured stragglers found via coverage
// once the scenario suites landed — pure helpers with real decision surface (output-type matching, data-URL
...
// and DETECT carry their own copies (a planned consolidation collapses all three into one engine). These pin that a
// failure at SetEnv or workspace resolution aborts the run as report.FAILED for those two behaviors too.
