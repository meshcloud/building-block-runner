...
// Test_TfBinaries_ConcurrentGetTF_IsRaceFree pins concurrency hazard H1 (plan-05 §7):
...
// existing mitigation under -race rather than rebuilding it (§7: "verify, don't rebuild").
...
func Test_TfBinaries_ConcurrentGetTF_IsRaceFree(t *testing.T) {
	...
	// Two concurrent callers per version (2x2, per §7's H1 test spec), each with its own
