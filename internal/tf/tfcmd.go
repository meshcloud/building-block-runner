...
type TfCmdParams struct {
	...
	// explicitly in place of the former AppConfig global. They are populated
...
func (tfcmd *GenericTfCmd) failWithUserMsg(err error, overrideUserMsg *string) {
	...
	if stepStatus == nil {
		...
		if errors.Is(tfcmd.ctx.Err(), context.DeadlineExceeded) {
			msgText = fmt.Sprintf("execution step stopped, because it exceeded the configured timeout of %d minutes", int(tfcmd.params.tfCommandTimeout.Minutes()))
			_, _ = tfcmd.runContextInfo.logwrap.PrintlnToUpdateLogs(msgText)
...
func (tfcmd *GenericTfCmd) init(tf TfFacade) error {
	...
	initCtx, cancel := context.WithTimeout(tfcmd.ctx, tfcmd.params.initTimeout)
	defer cancel()
...
func (tfcmd *GenericTfCmd) useWorkspaceIfNeeded(tf TfFacade) error {
	...
	wsCtx, cancel := context.WithTimeout(tfcmd.ctx, tfcmd.params.wsTimeout)
	defer cancel()
...
func (tfcmd *GenericTfCmd) selectWorkspace(tf TfFacade) (string, error) {
	...
	wsCtx, cancelList := context.WithTimeout(tfcmd.ctx, tfcmd.params.wsTimeout)
	defer cancelList()
...
func (tfcmd *GenericTfCmd) collectOutput(output map[string]tfexec.OutputMeta) {
	...
	parsedOutput := make(map[string]report.Output)
...
func (tfcmd *GenericTfCmd) buildTfEnv() (map[string]string, error) {
	...
	for varName, variable := range tfcmd.params.vars {
		...
		if variable.env && !strings.HasPrefix(varName, TfVarEnvPrefix) {
			encodedValue, err := encodeVarValueForEnv(variable.value, variable.Type)
			if err != nil {
				return nil, err
			}
...
func (tfcmd *GenericTfCmd) vars() error {
	...
	for varName, variable := range SortedByKeys(tfcmd.params.vars) {
		...
		if variable.Type == DATA_TYPE_CODE || variable.Type == DATA_TYPE_LIST {
			diags = varsFile.AddRawVariable(varName, fmt.Sprintf("%v", variable.value), opts)
		} else {
			diags = varsFile.AddVariable(varName, variable.value, opts)
		}
...
func (tfcmd *GenericTfCmd) setCurrentStepStatus(status report.ExecutionStatus) {
...
func (tfcmd *GenericTfCmd) nextStep() {
	...
	currentStep := tfcmd.runContextInfo.runStatus.CurrentStepStatus()
	if currentStep != nil {
		currentStep.Status = report.SUCCEEDED
	}
	...
	if err != nil {
		...
		tfcmd.runContextInfo.runStatus.CurrentStepStatus().Status = report.IN_PROGRESS
...
func (tfcmd *GenericTfCmd) setRunStatus(e report.ExecutionStatus) {
	tfcmd.runContextInfo.runStatus.Status = e
	tfcmd.Printfln("STATUS %s", e.String())
}

// commitStatus publishes an independent snapshot of the live runStatus into progress for the
// observer goroutine. Clone reallocates the Steps/Artifact slices so the observer's concurrent
// Snapshot can never race the work goroutine's in-place step mutations.
func (tfcmd *GenericTfCmd) commitStatus() {
	snapshot := tfcmd.runContextInfo.runStatus.Clone()
	tfcmd.runContextInfo.progress.Mutate(func(s *report.RunStatus) { *s = snapshot })
}
...
func (tfcmd *GenericTfCmd) startRun() {
	tfcmd.firstStep()
	tfcmd.setRunStatus(report.IN_PROGRESS)
	tfcmd.commitStatus()
}
...
func (tfcmd *GenericTfCmd) completeRun(userMessage *string) {
	...
	tfcmd.setCurrentStepStatus(report.SUCCEEDED)
	tfcmd.setRunStatus(report.SUCCEEDED)
	tfcmd.commitStatus()
}
