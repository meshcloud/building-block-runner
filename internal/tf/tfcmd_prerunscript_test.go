...
func makePreRunScriptTfCmd(t *testing.T, preRunScript *string, downloadTfBinaries bool) (*GenericTfCmd, func() string) {
	...
	sut := &GenericTfCmd{
		...
		runContextInfo: &RunContextInfo{
			...
			progress: report.NewProgress(report.RunStatus{}),
		},
		bin: &TfBinaries{
...
// requireBash skips the test on hosts without a bash binary on PATH, since
// runPreRunScript always execs "bash" (via exec.CommandContext) to run non-empty scripts.
