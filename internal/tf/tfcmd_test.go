...
func Test_collectOutput(t *testing.T) {
	...
	// report.Output.Type is a plain string (unlike the deleted tf-local TfOutput.Type DataType), so
	// collectOutput now stores the DataType stringified.
	assert.Equal(t, string(DATA_TYPE_INTEGER), collectedOutput["number"].Type)
	assert.Equal(t, jsonRawFrom(5), collectedOutput["number"].Value)

	assert.Equal(t, string(DATA_TYPE_CODE), collectedOutput["list"].Type)
	assert.Equal(t, "[\n  \"foo\",\n  \"bar\"\n]", collectedOutput["list"].Value)

	assert.Equal(t, string(DATA_TYPE_CODE), collectedOutput["object"].Type)
