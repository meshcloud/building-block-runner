...
func (w *Worker) tfExecution(ctx context.Context, run *Run) error {
	w.log.Info("Start execution of run", "behavior", run.Behavior.str(), "buildingBlock", run.BuildingBlockName)
	claimedAt := time.Now()
	...
	go w.workRoutine(execCtx, run, runContextInfo, &wg, workDone, &registerErr)
	...
	if ctx.Err() != nil {
		...
		if _, err := w.runApi.Report(runContextInfo.progress.Snapshot()); err != nil {
			w.log.Warn("failed to report ABORTED status on shutdown", "runId", run.Id, "error", err)
		}
	}
	...
	w.log.Info("Finished execution of run", "behavior", run.Behavior.str(), "buildingBlock", run.BuildingBlockName)
	w.log.Info("-----")
...
func (w *Worker) workRoutine(ctx context.Context, run *Run, runContextInfo *RunContextInfo, wg *sync.WaitGroup, doneSignallingChan chan struct{}, registerErr *error) {
	defer wg.Done()
