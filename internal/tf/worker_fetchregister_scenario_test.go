...
func (suite *WorkerTestSuite) Test_HandleFetchRunError() {
	...
	for _, tc := range cases {
		suite.Run(tc.name, func() {
			...
			// D12 (§4.3, step 4): only the "failed" branches (unexpected fetch errors) count
			// as a poll error -- "norun" (404/409/benign transport quirk) must not.
			wantPollErrors := 0
			if tc.want == failed {
				wantPollErrors = 1
			}
			suite.Equal(wantPollErrors, meter.snapshot().pollErrors, "poll-error count for %s", tc.name)
		})
	}
}
