...
func (suite *WorkerTestSuite) SetupSuite() {
	...
	suite.cfg = TfRunnerConfig{
		BaseConfig:            config.BaseConfig{Uuid: "scenario-runner"},
...
func (suite *WorkerTestSuite) runWorker() {
	...
	runJsonBase64 := base64.StdEncoding.EncodeToString(body)
	...
	suite.scenarioAuth.runToken = &dto.Spec.RunToken

	suite.w.meterOrNoop().RunClaimed()
	_ = suite.w.tfExecution(context.Background(), &dto, behavior, impl, source, vars, runJsonBase64)
}
