...
func withTestAppConfig(t *testing.T, runApiUrl string) {
	t.Helper()
	previous := tfrun.AppConfig
	t.Cleanup(func() { tfrun.AppConfig = previous })
	tfrun.AppConfig = tfrun.TfRunnerConfig{
		...
		WsTimeoutMins:        1,
		InitTimeoutMins:      1,
		RunApiBackend:        tfrun.RunApiConfig{Url: runApiUrl},
	}
}
...
func Test_ExecuteSingleRun_MissingRunJsonFilePathEnv_ExitsNonZero(t *testing.T) {
	...
	tfbin, err := tfrun.ForTestNewTfBin(t.TempDir(), io.Discard, nil)
	require.NoError(t, err)

	code := executeSingleRun(logger, tfbin, tfrun.NoopDecryptor{})
