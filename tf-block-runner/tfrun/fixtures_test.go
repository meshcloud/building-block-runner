...
// CP1 (PLAN_DETAIL_01_tf_characterization_tests.md §9): shared, hermetic scenario-test fixtures.
// These replace the scenario suites' dependency on a real `https://github.com/meshcloud/
// meshstack-hub.git` clone (F1) with an on-disk git repository, and give every later checkpoint
// (CP2-13) one builder for fixture RunDetailsDTOs and one seam for genuinely encrypting/decrypting
...
// --- RunDetailsDTO fixture builder ------------------------------------------------------------

// runDetailsFixture is the mutable state runDetailsOptions apply to; runDetailsDTO/
// runDetailsFetchCall seed it with the defaults the pre-CP1 mock*FetchCall helpers hardwired
...
// withRepo sets the git source: url is a remote URL or (CP1) a localGitRepo.Path; path is the
...
// runDetailsDTO builds a fixture RunDetailsDTO shaped like a real meshfed API response
// (RunSpecDTO/TerraformImplementation/BuildingBlockSpecDTO), configured via opts. The zero value
// (no opts) reproduces the minimal APPLY-without-artifact run the pre-CP1 mockValidRunDetailsFetchCall
// built.
func runDetailsDTO(opts ...runDetailsOption) *meshapi.RunDetailsDTO {
	f := runDetailsFixture{
		behavior: APPLY.str(),
	...
	// this concrete, plain-old-data struct; discarding the error mirrors the pre-CP1 fixture code.
...
// testCrypto loads the repo's checked-in test key pair (tf-block-runner/resources/test.pem +
// test.key — the same material go-meshapi-client/crypto/meshcertbasedcrypto_test.go proves
...
// that will consume them in later checkpoints (CP2-13) — the option constructors below have no
// other caller yet within this checkpoint.
